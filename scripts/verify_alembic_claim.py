...
3. POST to the coord service's ``/coord/claims/check?kind=alembic_revision``
   endpoint, with the down_revision and the revision id, and verify a
   matching claim exists in ``coord.claims_audit`` (acquired by SOMEONE
   for the same down_revision the file declares).
...
def check_claim(
    ...
    revision being authored. Returns (ok, message)."""
    body = json.dumps(
        {
            "kind": "alembic_revision",
            "resource_key": revision_id,
    ...
    for path in added:
        ...
        if not ok:
            failures.append(
                ...
                f"acquired coord claim. Reason: {reason}\n"
                f"    Acquire before authoring:\n"
                f"        coord.py claim acquire \\\n"
                f"            --kind alembic_revision \\\n"
                f"            --resource '{rev}' \\\n"
                f'            --metadata \'{{"down_revision":"{down}","repo":"qontinui-web"}}\'\n'
