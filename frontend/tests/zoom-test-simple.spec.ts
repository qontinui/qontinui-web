...
test('screenshot-zoom-simple', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Extract Images Cut pattern images from screenshots' }).click();
  await expect(page.getByRole('heading', { name: 'Image Extraction', level: 1 })).toBeVisible({ timeout: 10000 });

  // Click "Capture Screen" button to expand options
  await page.getByRole('button', { name: /capture screen/i }).first().click();
  await expect(page.getByRole('button', { name: /#1 Left 1920/i })).toBeVisible({ timeout: 5000 });
  ...
  // Get initial zoom
  const initialZoom = parseFloat(await zoomIndicator.getAttribute('data-zoom-value') || '0');
  ...
  const afterZoom = parseFloat(await zoomIndicator.getAttribute('data-zoom-value') || '0');
  ...
  // Log the comparison
  console.log(`COMPARISON: initial=${initialZoom}, after=${afterZoom}, increased=${afterZoom > initialZoom}`);
