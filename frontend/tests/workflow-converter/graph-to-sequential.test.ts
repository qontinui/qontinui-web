...
import {
  ...
  Action,
  Connections,
} from "../../src/lib/action-schema/action-types";
import {
...
describe("GraphToSequentialConverter", () => {
  ...
  describe("Basic Conversion", () => {
    test("should convert simple linear graph to sequential", () => {
      const action1 = createTestAction("CLICK", "action-1", {
        target: { image: "test.png" },
      });
      const action2 = createTestAction("TYPE", "action-2", { text: "hello" });
      const action3 = createTestAction("FIND", "action-3", {
        target: { image: "result.png" },
        strategy: "FIRST",
      });

      const workflow = createTestWorkflow([action1, action2, action3], {
        "action-1": {
    ...
    test("should preserve action IDs by default", () => {
      const action1 = createTestAction("CLICK", "my-custom-id", {
        target: { image: "test.png" },
      });
  ...
  describe("Validation", () => {
    test("should throw error for non-graph workflow", () => {
      ...
      expect(() => converter.convert(workflow)).toThrow(
        WorkflowValidationError
      );
    });
  ...
  describe("IF Pattern Reconstruction", () => {
    test("should reconstruct simple IF action", () => {
      ...
      const thenAction = createTestAction("CLICK", "then-1", {
        target: { image: "yes.png" },
      });
      const elseAction = createTestAction("CLICK", "else-1", {
        target: { image: "no.png" },
      });

      const workflow = createTestWorkflow([ifAction, thenAction, elseAction], {
        "if-1": {
          main: [
      ...
      expect(
        (result[0].config as Record<string, unknown>).thenActions
      ).toContain("then-1");
      expect(
        (result[0].config as Record<string, unknown>).elseActions
      ).toContain("else-1");
    });
    ...
    test("should reconstruct IF with multiple actions in branches", () => {
      ...
      const then1 = createTestAction("CLICK", "then-1", {
        target: { image: "a.png" },
      });
      const then2 = createTestAction("TYPE", "then-2", { text: "yes" });
      const else1 = createTestAction("CLICK", "else-1", {
        target: { image: "b.png" },
      });

      const workflow = createTestWorkflow([ifAction, then1, then2, else1], {
        "if-1": {
          main: [
    ...
    test("should reconstruct nested IF actions", () => {
      ...
      const innerThen = createTestAction("CLICK", "inner-then", {
        target: { image: "a.png" },
      });
      const innerElse = createTestAction("CLICK", "inner-else", {
        target: { image: "b.png" },
      });
      const outerElse = createTestAction("CLICK", "outer-else", {
        target: { image: "c.png" },
      });
  ...
  describe("LOOP Pattern Reconstruction", () => {
    test("should reconstruct simple LOOP action", () => {
      ...
      const bodyAction = createTestAction("CLICK", "body-1", {
        target: { image: "test.png" },
      });

      const workflow = createTestWorkflow([loopAction, bodyAction], {
      ...
      expect((result[0].config as Record<string, unknown>).actions).toContain(
        "body-1"
      );
    });
    ...
    test("should reconstruct LOOP with multiple body actions", () => {
      ...
      const body1 = createTestAction("CLICK", "body-1", {
        target: { image: "a.png" },
      });
      const body2 = createTestAction("TYPE", "body-2", { text: "test" });
      const body3 = createTestAction("FIND", "body-3", {
        target: { image: "element.png" },
        strategy: "FIRST",
      });

      const workflow = createTestWorkflow([loopAction, body1, body2, body3], {
        "loop-1": { main: [[{ action: "body-1", type: "main", index: 0 }]] },
    ...
    test("should reconstruct nested LOOP actions", () => {
      ...
      const innerBody = createTestAction("CLICK", "inner-body", {
        target: { image: "a.png" },
      });

      const workflow = createTestWorkflow([outerLoop, innerLoop, innerBody], {
        "outer-loop": {
          main: [[{ action: "inner-loop", type: "main", index: 0 }]],
        },
        "inner-loop": {
          main: [[{ action: "inner-body", type: "main", index: 0 }]],
        },
        "inner-body": {
          main: [[{ action: "inner-loop", type: "main", index: 0 }]],
        },
  ...
  describe("Complex Nested Patterns", () => {
    test("should reconstruct IF inside LOOP", () => {
      ...
      const thenAction = createTestAction("CLICK", "then-1", {
        target: { image: "even.png" },
      });
      const elseAction = createTestAction("CLICK", "else-1", {
        target: { image: "odd.png" },
      });
...
describe("LinearizabilityChecker", () => {
  ...
  test("should fail on multiple entry points", () => {
    ...
    expect(result.linearizable).toBe(false);
    expect(result.issues.some((i) => i.includes("Multiple entry points"))).toBe(
      true
    );
  });
  ...
  test("should fail on parallel execution", () => {
    ...
    expect(result.linearizable).toBe(false);
    expect(result.issues.some((i) => i.includes("Parallel execution"))).toBe(
      true
    );
  });
...
describe("PatternDetector", () => {
  ...
  describe("IF Pattern Detection", () => {
    ...
    test("should return null for non-IF action", () => {
      const action = createTestAction("CLICK", "click-1", {
        target: { image: "test.png" },
      });
  ...
  describe("Pattern Collection", () => {
    test("should detect all IF patterns in workflow", () => {
      ...
      const then2 = createTestAction("CLICK", "then-2", {
        target: { image: "c.png" },
      });
      const else2 = createTestAction("CLICK", "else-2", {
        target: { image: "d.png" },
      });
    ...
    test("should detect all LOOP patterns in workflow", () => {
      ...
      const body2 = createTestAction("CLICK", "body-2", {
        target: { image: "b.png" },
      });

      const workflow = createTestWorkflow([loop1, loop2, body1, body2], {
        "loop-1": { main: [[{ action: "body-1", type: "main", index: 0 }]] },
        "body-1": { main: [[{ action: "loop-1", type: "main", index: 0 }]] },
...
describe("Error Handling", () => {
  test("NonLinearWorkflowError should contain issues", () => {
    const error = new NonLinearWorkflowError("Test error", [
      "Issue 1",
      "Issue 2",
    ]);
