...
test.describe('Navigation Test Generator', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector('h2:has-text("Navigation Test Generator")', { timeout: 30000 });
  });
  ...
  test('should display page with all sections', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: /State Graph/i })).toBeVisible();
    await expect(page.getByRole('button', { name: /Test Specs/i })).toBeVisible();
  ...
  test('should have Start Exploration button disabled without URL', async ({ page }) => {
    const startButton = page.getByRole('button', { name: /Start Exploration/i });
  ...
  test('should send correct API request format', async ({ page }) => {
    ...
    expect((capturedRequest as Record<string, unknown>)?.target_url).toBeUndefined();
    expect((capturedRequest as Record<string, unknown>)?.max_elements).toBeUndefined();
  });
  ...
  test('should show Source selector with New Exploration button', async ({ page }) => {
    // Verify source selector is present
    await expect(page.getByText('Source:')).toBeVisible();
    await expect(page.getByRole('button', { name: /New Exploration/i })).toBeVisible();
  });
});
