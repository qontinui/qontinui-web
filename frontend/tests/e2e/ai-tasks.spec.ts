...
test.describe('AI Tasks Page', () => {
  ...
  async function getProjectId(page: import('@playwright/test').Page): Promise<string | null> {
  ...
  test('should load AI tasks page without errors', async ({ page }) => {
    ...
    const hasTaskHistoryHeading = await page.locator('text=Task History').count() > 0;
    expect(hasAITasksHeading || hasTaskHistoryHeading).toBeTruthy();
  });

  test('should display content when navigating via sidebar AI Tasks menu', async ({ page }) => {
    ...
    // Log current state
    const hasEmptyState = await page.locator('text=No AI tasks found').count() > 0;
    const hasErrorMessage = await page.locator('text=Error loading').count() > 0;
    const hasTasks = await page.locator('table tbody tr').count() > 0;
    const hasLoading = await page.locator('text=Loading').count() > 0;
    const hasNoProjectSelected = await page.locator('text=No project selected').count() > 0;
