...
test.describe('Web Extraction Page', () => {
  ...
  test('displays Connect Runner link when no runner is connected', async ({ page }) => {
    ...
    const runnerCardTitle = page.locator('text=Select a connected runner to perform the extraction');
    ...
    const isNoRunners = await noRunnersAlert.isVisible({ timeout: 3000 }).catch(() => false);

    if (isNoRunners) {
      // Verify the "Go to Connect Runner" link is displayed
      const connectRunnerLink = page.locator('a:has-text("Go to Connect Runner")');
      ...
      await expect(connectRunnerLink).toHaveAttribute('href', new RegExp(`/connect-runner.*project=${PROJECT_ID}`), { timeout: 10000 });
    } else {
      ...
      const runnerSelect = page.locator('[role="combobox"]:has-text("Select a runner")');
