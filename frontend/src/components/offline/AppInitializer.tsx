  ...
  useEffect(() => {
    ...
    const initializeApp = async () => {
      try {
        ...
        console.log(
          `[AppInitializer] Cleaned up ${deletedScreenshots} expired screenshots`
        );

        // 2. Cleanup old completed sync items
        const deletedSyncItems = await syncQueue.clearCompleted();
        console.log(
          `[AppInitializer] Cleaned up ${deletedSyncItems} completed sync items`
        );
        ...
        if (navigator.onLine) {
          ...
          console.log(
            `[AppInitializer] Processed ${processed} pending items from previous session`
          );
        } else {
          console.log(
            "[AppInitializer] Offline - will process pending items when connection is restored"
          );
        }
