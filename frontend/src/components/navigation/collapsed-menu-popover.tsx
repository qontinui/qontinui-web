...
export function CollapsedMenuPopover({
  ...
  useEffect(() => {
    ...
    const parentButton = document.querySelector(`[data-nav-id="${parentId}"]`) as HTMLElement
    if (parentButton) {
      const rect = parentButton.getBoundingClientRect()
      const parentZIndex = window.getComputedStyle(parentButton.parentElement!).zIndex
      setPosition({
      ...
      setTimeout(() => {
        const popoverElements = document.querySelectorAll('[data-popover="true"]')
  ...
  const popoverContent = (
    <>
      ...
      >
      {children.map((child) => {
        const childIconColor = parentId === 'create' ? parentColor : child.color
        ...
        return (
          <button
            ...
            title={child.label}
          >
            <span style={{ color: childIconColor, fontSize: '20px' }}>{child.icon}</span>
          </button>
        )
      })}
    </div>
