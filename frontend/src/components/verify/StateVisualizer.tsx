...
export function StateVisualizer({
  ...
  useEffect(() => {
    ...
    state.stateImages?.forEach((stateImage) => {
      stateImage.patterns?.forEach((pattern) => {
        if (pattern.fixed && pattern.imageId && !imageMap.has(pattern.imageId)) {
  ...
  const redraw = useCallback(() => {
    ...
    ctx.restore()
  }, [state, zoom, pan, showPositions, highlightElement, loadedImages, canvasSize])
  ...
  const drawStateElements = (ctx: CanvasRenderingContext2D) => {
    ...
    state.regions?.forEach((region) => {
      ...
      ctx.strokeStyle = isHighlighted ? '#3b82f6' : (region.isSearchRegion ? '#10b981' : '#22c55e')
      ...
      if (showPositions) {
        ...
        ctx.font = '10px Arial'
        ctx.fillText(`(${region.x}, ${region.y})`, region.x + 5, region.y + region.height - 5)
      }
    })
    ...
    state.stateImages?.forEach((stateImage) => {
      stateImage.patterns?.forEach((pattern) => {
        if (pattern.fixed && pattern.offsetX !== undefined && pattern.offsetY !== undefined) {
          ...
          if (img) {
            ...
            // Draw label
            ctx.fillStyle = '#ffffff'
            ctx.fillRect(pattern.offsetX, pattern.offsetY - 18, ctx.measureText(stateImage.name).width + 10, 16)
            ctx.fillStyle = '#1e293b'
            ctx.font = 'bold 11px Arial'
            ctx.fillText(stateImage.name, pattern.offsetX + 5, pattern.offsetY - 5)
            ...
            const width = 100
            const height = 100
            ctx.fillStyle = isHighlighted ? 'rgba(59, 130, 246, 0.1)' : 'rgba(148, 163, 184, 0.1)'
            ...
            ctx.fillText(stateImage.name, pattern.offsetX + width / 2, pattern.offsetY + height / 2)
            ctx.textAlign = 'left'
    ...
    state.locations?.forEach((location) => {
      ...
      const color = location.fixed ? '#dc2626' : location.anchor ? '#8b5cf6' : '#f59e0b'
      ...
      if (isHighlighted || showPositions) {
        ctx.fillStyle = '#ffffff'
        ctx.fillRect(location.x + 10, location.y - 8, ctx.measureText(location.name).width + 10, 16)
        ctx.fillStyle = '#1e293b'
        ...
        if (showPositions) {
          ctx.fillStyle = '#64748b'
          ctx.font = '9px Arial'
          ctx.fillText(`(${location.x}, ${location.y})`, location.x + 15, location.y + 14)
        }
      }
    })
  }
  ...
  return (
    <div className="relative flex flex-col h-full">
      ...
      <div className="absolute top-4 right-4 z-10 flex flex-col gap-2 bg-background/80 backdrop-blur-sm rounded-lg p-2 shadow-lg">
        ...
        <Button size="sm" variant="outline" onClick={handleResetView} title="Fit to View">
      ...
      {/* Canvas */}
      <div ref={containerRef} className="flex-1 relative overflow-hidden rounded-lg border bg-white">
        <canvas
          ref={canvasRef}
      ...
      <div className="mt-2 text-sm text-muted-foreground text-center">
        Drag to pan • Scroll to zoom • Click elements in metadata panel to highlight
      </div>
    </div>
  )
}
