    ...
    <TooltipProvider>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex items-center justify-between">
            <Tooltip>
              ...
              <TooltipContent side="right" className="max-w-xs">
                <p className="text-xs">
                  The confidence score indicates how reliably this state can be identified.
          ...
          <div className="grid grid-cols-2 gap-2 pt-2">
            <div className="text-center p-2 bg-gray-50 rounded">
              <p className="text-lg font-semibold">{state.stateImageIds?.length || 0}</p>
              <p className="text-xs text-gray-600">StateImages</p>
            </div>
            <div className="text-center p-2 bg-gray-50 rounded">
              <p className="text-lg font-semibold">{state.screenshotIds?.length || 0}</p>
      ...
      <Tabs defaultValue="stateimages" className="flex-1">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="stateimages" className="flex items-center gap-1">
            <ImageIcon className="mr-1 h-4 w-4" />
            <span>StateImages</span>
            <span className="ml-1 text-gray-600">({stateImageObjects.length})</span>
          </TabsTrigger>
          <TabsTrigger value="screenshots" className="flex items-center gap-1">
            <Camera className="mr-1 h-4 w-4" />
            <span>Screenshots</span>
            <span className="ml-1 text-gray-600">({state.screenshotIds?.length || 0})</span>
          </TabsTrigger>
        </TabsList>
          ...
          <Card>
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                ...
                >
                  {showAllStateImages ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                </Button>
              </div>
            </CardHeader>
            <CardContent>
                ...
                <div className="space-y-2">
                  {stateImageObjects.map((stateImage) => (
                    <div
                      ...
                      onClick={() => onHighlightStateImages?.([stateImage.id])}
                    >
                      <div className="flex gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-gray-600 mt-1">
                            Size: {stateImage.x2 - stateImage.x + 1} × {stateImage.y2 - stateImage.y + 1}
                          </div>
                          <div className="text-xs text-gray-500 mt-1">
                            Frequency: {(stateImage.frequency * 100).toFixed(1)}%
                          </div>
                        </div>
                      </div>
                    </div>
