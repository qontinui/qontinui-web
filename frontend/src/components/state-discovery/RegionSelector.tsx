...
type DragHandle = 'none' | 'move' | 'nw' | 'n' | 'ne' | 'e' | 'se' | 's' | 'sw' | 'w';
...
const RegionSelector: React.FC<RegionSelectorProps> = ({
  ...
  const [currentRegion, setCurrentRegion] = useState<{ x: number; y: number; width: number; height: number } | null>(initialRegion || null);
  ...
  const getHandleAtPosition = useCallback((x: number, y: number): DragHandle => {
    ...
    if (Math.abs(x - (rx + rw)) < handleSize && Math.abs(y - ry) < handleSize) return 'ne';
    if (Math.abs(x - (rx + rw)) < handleSize && Math.abs(y - (ry + rh)) < handleSize) return 'se';
    if (Math.abs(x - rx) < handleSize && Math.abs(y - (ry + rh)) < handleSize) return 'sw';

    // Check edges
    if (Math.abs(x - rx) < handleSize && y > ry && y < ry + rh) return 'w';
    if (Math.abs(x - (rx + rw)) < handleSize && y > ry && y < ry + rh) return 'e';
    if (Math.abs(y - ry) < handleSize && x > rx && x < rx + rw) return 'n';
    if (Math.abs(y - (ry + rh)) < handleSize && x > rx && x < rx + rw) return 's';
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (isSelecting) {
      ...
      switch (dragHandle) {
        case 'move':
          newRegion.x = Math.max(0, Math.min(imageWidth - newRegion.width, currentRegion.x + dx));
          newRegion.y = Math.max(0, Math.min(imageHeight - newRegion.height, currentRegion.y + dy));
          break;
        case 'nw':
          newRegion.x = Math.min(currentRegion.x + currentRegion.width - 10, currentRegion.x + dx);
          newRegion.y = Math.min(currentRegion.y + currentRegion.height - 10, currentRegion.y + dy);
  ...
  }, [isSelecting, isDragging, startPoint, currentRegion, dragHandle, getRelativePosition, getHandleAtPosition, imageWidth, imageHeight]);
      ...
      <div className="flex items-center justify-between">
        ...
        {currentRegion && (
          <div className="text-sm text-gray-600">
            ...
            {Math.round(currentRegion.width)}×{Math.round(currentRegion.height)}px
          </div>
        )}
      </div>
      ...
      <Card>
        <CardContent className="p-0">
          <div
            ...
            style={{
              height: '400px',
              cursor: isSelecting ? 'crosshair' : (isDragging ? 'move' : 'crosshair'),
              userSelect: 'none',
              minHeight: '400px'
            }}
          ...
          >
            ...
            {currentRegion && (
              ...
              >
                {/* Resize handles */}
                <div className="absolute -left-1 -top-1 w-2 h-2 bg-blue-500 cursor-nw-resize" style={{ pointerEvents: 'auto' }} />
                <div className="absolute left-1/2 -top-1 w-2 h-2 bg-blue-500 cursor-n-resize -translate-x-1/2" style={{ pointerEvents: 'auto' }} />
                <div className="absolute -right-1 -top-1 w-2 h-2 bg-blue-500 cursor-ne-resize" style={{ pointerEvents: 'auto' }} />
                <div className="absolute -right-1 top-1/2 w-2 h-2 bg-blue-500 cursor-e-resize -translate-y-1/2" style={{ pointerEvents: 'auto' }} />
                <div className="absolute -right-1 -bottom-1 w-2 h-2 bg-blue-500 cursor-se-resize" style={{ pointerEvents: 'auto' }} />
                <div className="absolute left-1/2 -bottom-1 w-2 h-2 bg-blue-500 cursor-s-resize -translate-x-1/2" style={{ pointerEvents: 'auto' }} />
                <div className="absolute -left-1 -bottom-1 w-2 h-2 bg-blue-500 cursor-sw-resize" style={{ pointerEvents: 'auto' }} />
                <div className="absolute -left-1 top-1/2 w-2 h-2 bg-blue-500 cursor-w-resize -translate-y-1/2" style={{ pointerEvents: 'auto' }} />
      ...
      <Card className="bg-blue-50">
        <CardContent className="py-3">
          <p className="text-sm text-gray-600">
            <strong>Select Analysis Region:</strong> Click and drag to draw a rectangle around the area you want to analyze.
            Drag edges or corners to resize. Drag inside to move. This speeds up analysis by focusing on specific UI areas.
          </p>
        </CardContent>
      </Card>
    </div>
