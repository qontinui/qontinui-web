...
export function DirectPatternRegionSelector({
  ...
  const getRelativePosition = useCallback((e: React.MouseEvent): { x: number; y: number } | null => {
    ...
    const x = Math.max(0, Math.min(imageDimensions.width, (mouseX - imageLeft) / scale));
    const y = Math.max(0, Math.min(imageDimensions.height, (mouseY - imageTop) / scale));
  ...
  return (
    <div className="space-y-3">
      ...
      >
        ...
        {currentRegion && !isDrawing && renderRegion(currentRegion, '#3B82F6', 'Selected')}

        {/* Temporary drawing region (blue dashed) */}
        {isDrawing && tempRegion && renderRegion(tempRegion, '#60A5FA', 'Drawing')}
      ...
      <div className="text-xs text-gray-600 bg-blue-50 p-2 rounded">
        <strong>Instructions:</strong> Click and drag to select a region. Previously extracted
        regions are shown in gray. The current selection is shown in blue.
      </div>
    </div>
  );
}
