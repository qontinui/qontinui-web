...
export class TransitionAnimationController {
  ...
  loadTransition(
    ...
    if (process.env.NODE_ENV === "development") {
      logger.info("[TransitionAnimation] loadTransition: loaded", {
  ...
  private actionToAnimationConfig(
    ...
    switch (action.type) {
      ...
      case "MOUSE_UP": {
        if (process.env.NODE_ENV === "development") {
          logger.info(`[TransitionAnimation] ${action.type} action config:`, {
            actionId: action.id,
      ...
      case "RAG_FIND": {
        ...
        if (process.env.NODE_ENV === "development") {
          logger.info(`[TransitionAnimation] ${action.type} extracted IDs:`, {
            targetConfig,
            stateImageId,
            imageId,
          });
    ...
    if (process.env.NODE_ENV === "development") {
      ...
      logger.info(
        `[TransitionAnimation] findStateImageByImageAssetId: searching for "${imageAssetId}"`,
        {
          ...
          allPatternImageIds: allPatternImageIds.slice(0, 10),
  ...
  private resolveClickTargetPosition(
    ...
    if (process.env.NODE_ENV === "development") {
      logger.info("[TransitionAnimation] resolveClickTargetPosition called", {
        target,
  ...
  play(): void {
    if (process.env.NODE_ENV === "development") {
      logger.info("[TransitionAnimation] play() called", {
        hasData: !!this.data,
  ...
  private animate = (): void => {
    ...
    if (shouldLog) {
      logger.info("[TransitionAnimation] animate frame:", {
    ...
    if (this.state.isPlaying && this.state.phase !== "completed") {
      ...
      logger.info("[TransitionAnimation] Animation loop ended:", {
        phase: this.state.phase,
  ...
  private advancePhase(): void {
    ...
    if (process.env.NODE_ENV === "development") {
      logger.info("[TransitionAnimation] advancePhase:", {
        from: prevPhase,
        to: this.state.phase,
        prevActionIndex,
