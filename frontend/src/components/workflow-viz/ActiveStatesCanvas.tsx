...
export function ActiveStatesCanvas({
  ...
  const redraw = useCallback(() => {
    ...
    states.forEach((state, index) => {
      ...
      ctx.fillText(`z: ${index + 1}`, bounds.x + 10, bounds.y + bounds.height - 10)
  ...
  // Draw state elements
  const drawStateElements = (ctx: CanvasRenderingContext2D, state: State, highlighted: boolean) => {
      ...
      {states.length > 0 && (
        <div className="absolute top-4 left-4 z-10 bg-background/90 backdrop-blur-sm rounded-lg p-3 shadow-lg">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium">{states.length} Active State(s)</span>
          </div>
        </div>
      )}
        ...
        {states.length === 0 ? (
          <div className="flex items-center justify-center h-full">
            <div className="text-center text-muted-foreground">
              ...
              <p className="text-xs mt-1">States will appear as actions execute</p>
            </div>
          </div>
        ) : (
          <canvas
