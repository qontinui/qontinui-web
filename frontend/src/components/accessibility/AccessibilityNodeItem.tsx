...
import { ChevronDown, ChevronRight, MousePointerClick, Copy } from "lucide-react";
import { cn } from "@/lib/utils";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import type { AccessibilityNode, AccessibilityRole } from "@qontinui/schemas/accessibility";
...
const ROLE_COLORS: Record<string, string> = {
  ...
  textbox: "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300",
  checkbox: "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300",
  radio: "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300",
  ...
  menuitem: "bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-300",
  ...
  navigation: "bg-indigo-100 text-indigo-800 dark:bg-indigo-900/30 dark:text-indigo-300",
};

function getRoleColor(role: AccessibilityRole): string {
  return ROLE_COLORS[role] ?? "bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-300";
}
  ...
  return (
    <div data-slot="accessibility-node">
      ...
      >
        ...
        {/* Value */}
        {node.value && (
          <span className="text-xs text-muted-foreground truncate max-w-[100px]" title={node.value}>
            = {node.value}
          </span>
        )}
