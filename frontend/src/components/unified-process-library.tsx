...
export function UnifiedProcessLibrary({
  ...
  const workflowCategories = [...new Set(workflows.map(w => w.category || "Main"))]
  ...
  const itemsByCategory = allCategories.reduce((acc, category) => {
    acc[category] = workflows.filter(w => (w.category || "Main") === category)
    return acc
  ...
  const handleDeleteCategory = (category: string) => {
    const workflowsInCategory = workflows.filter(w => (w.category || "Main") === category)
            ...
            <div className="flex items-center gap-1.5 flex-1 min-w-0">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <h4 className="font-medium text-xs truncate">{getItemName(item)}</h4>
                </TooltipTrigger>
                <TooltipContent>
    ...
    <TooltipProvider>
      ...
      <div className="space-y-2">
        {allCategories.map(category => {
          ...
          const isDeletable = category !== 'Main' && category !== 'Transitions'
      ...
      <DeleteCategoryDialog
        ...
        onClose={() => setDeleteCategoryDialog({ open: false, category: "", workflows: [] })}
