...
const CONSOLE_SCOPE = [
  ...
  "app/(app)/admin/coord/",
  // The consolidated sessions console (`/sessions`) files its tables beside
  // its own surface rather than under `admin/coord/`; without this entry its
  // registered rows read as phantoms.
  "components/sessions/",
];
...
describe("the console surface registry is self-enforcing", () => {
  ...
  it("gives every registered table a distinct row", () => {
    // Uniqueness, not order — the registry is grouped by wave for a reader,
    // and two rows holding one table would let a stale palette hide behind a
    // fresh one while the set-equality clause above still passed. Keyed on the
    // table OBJECT, not the module path: one module may legitimately declare
    // two orthogonal tables (the sessions console's liveness and work axes),
    // and each gets its own row.
    const tables = CONSOLE_PALETTES.map((s) => s.attentionByKind);
