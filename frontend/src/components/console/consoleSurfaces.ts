 ...
 * import list is its whole point: it reaches into every surface's status module,
...
// The agent-text-units console files its table in a `_lib/` module rather than
// a `*Status.ts`, and was audited only beside itself (`unitRows.test.ts`)
// until the source scan in `consoleSurfaces.test.ts` found it.
import {
  ATTENTION_BY_KIND as AGENT_TEXT_UNIT_ATTENTION,
...
export const CONSOLE_PALETTES: ReadonlyArray<ConsoleSurface> = [
  ...
  },
  {
    ...
    module: "app/(app)/admin/coord/decision-policies/decisionPolicyStatus.ts",
  ...
  },
  {
    ...
    module: "app/(app)/admin/coord/gates/continuationStatus.ts",
  ...
  {
    surface: "agent text units (/admin/coord/agent-text-units)",
    module: "app/(app)/admin/coord/_agent-text-units/_lib/unitRows.ts",
    attentionByKind: AGENT_TEXT_UNIT_ATTENTION,
    palette: {
  ...
  // --- the consolidated sessions console ------------------------------------
  {
    ...
    module: "components/sessions/sessionConsoleStatus.ts",
  ...
  // `2026-09-01-session-finished-marker-and-unfinished-resume` Phase 4. It
  // names the SAME module as the row above, which is why the uniqueness clause
  // in `consoleSurfaces.test.ts` is keyed on the table, not the module.
  {
