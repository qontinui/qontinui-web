...
import type { Region, OptimizationScreenshot } from "@/types/pattern-optimization"
...
export function RegionSelector({ screenshot }: RegionSelectorProps) {
  ...
  const drawCanvas = useCallback(() => {
    ...
    if (currentRegion || screenshot?.region) {
      ...
      if (region) {
        ...
        ctx.fillText(`${Math.round(region.width)}×${Math.round(region.height)}`, x + 2, y - 4)
      }
    }
    ...
    if (showCrosshair && mousePos) {
      ...
      ctx.fillStyle = 'black'
      ctx.font = '12px monospace'
      ctx.fillText(`${Math.round(mousePos.x / zoom)}, ${Math.round(mousePos.y / zoom)}`, mousePos.x + 15, mousePos.y - 5)
    }
  ...
  const handleMouseUp = useCallback(() => {
    ...
    if (currentRegion && screenshot && currentRegion.width > 5 && currentRegion.height > 5) {
    ...
    return (
      <Card className="h-full flex items-center justify-center bg-[#27272A]/50 border-gray-700">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-400">Select a screenshot to define region</p>
        </div>
      </Card>
    )
  }
        ...
        <div className="p-2 bg-[#27272A]/50 rounded">
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="font-mono text-[#00D9FF]">
              {Math.round(currentRegion.x)}, {Math.round(currentRegion.y)} • {Math.round(currentRegion.width)}×{Math.round(currentRegion.height)}
            </span>
          </div>
        </div>
      )}
