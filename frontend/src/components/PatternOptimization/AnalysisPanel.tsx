...
import { StateImageCreationDialog, StateImageConfig } from "./StateImageCreationDialog"
...
import type { OptimizationStrategy, ExtractedPattern } from "@/types/pattern-optimization"
import type { StateImage, SearchRegion } from "@/contexts/automation-context/types"
...
export function AnalysisPanel() {
  ...
  const handleEvaluateStrategy = useCallback(async (type: OptimizationStrategy['type']) => {
    setEvaluating(type)
    try {
  ...
  const handleStateImageCreation = useCallback(async (config: StateImageConfig) => {
    // Creating StateImages with config
    if (!analysis || !selectedStrategy) return

    try {
      // Get selected patterns
      const patternsToUse = analysis.extractedPatterns.filter(p => selectedPatterns.has(p.id))
      ...
      const stateImages: StateImage[] = patternsToUse.map((pattern, idx) => {
        const searchRegions: SearchRegion[] = config.includeSearchRegions && pattern.region
          ? [{
  ...
  }, [analysis, selectedStrategy, selectedPatterns, states, addState, updateState])

  const canAnalyze = session?.screenshots.some(s => s.label === 'positive' && s.region)

  // AnalysisPanel state tracked

  // Calculate strategy scores
  const strategyTypes: OptimizationStrategy['type'][] = ['multi-pattern', 'consensus', 'feature-based', 'differential']
  const bestStrategy = evaluations.reduce((best, current) => {
    if (!best) return current
    const currentScore = current.performance.truePositiveRate - current.performance.falsePositiveRate
    const bestScore = best.performance.truePositiveRate - best.performance.falsePositiveRate
    ...
    <div className="flex flex-col h-full space-y-4 overflow-hidden">
      ...
      <div className="flex items-center justify-between flex-shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            disabled={!selectedStrategy || !analysis || selectedPatterns.size === 0}
            className="border-gray-700 hover:border-[#BD00FF]"
            title={selectedPatterns.size === 0 ? "Select patterns first" : "Create StateImages from selected patterns"}
          >
      ...
      {!analysis ? (
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col overflow-hidden">
          ...
          <TabsContent value="overview" className="flex-1 space-y-4 overflow-y-auto p-2">
            {/* Statistics */}
            <Card className="bg-[#27272A]/50 border-gray-700">
              ...
              <CardContent className="space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-xs text-gray-400">Patterns Extracted</span>
                  <span className="text-sm font-bold">{analysis.extractedPatterns.length}</span>
                </div>
                <div className="flex justify-between items-center">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-bold">
                    {(analysis.statistics.minSimilarity * 100).toFixed(1)}% - {(analysis.statistics.maxSimilarity * 100).toFixed(1)}%
                  </span>
                </div>
                ...
                <CardContent>
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-medium">{bestStrategy.strategy.type}</span>
                      <Badge
                        variant={bestStrategy.recommendations.confidenceLevel === 'high' ? 'default' : 'secondary'}
                        className={cn(
                          bestStrategy.recommendations.confidenceLevel === 'high' && "bg-green-500"
                        )}
                      >
                    ...
                    <div className="text-xs text-gray-400">
                      <span title="True Positive Rate - How often it correctly finds the pattern">
                        TPR: {(bestStrategy.performance.truePositiveRate * 100).toFixed(1)}%
                      </span>
                      {' • '}
                      <span title="False Positive Rate - How often it incorrectly matches">
                        FPR: {(bestStrategy.performance.falsePositiveRate * 100).toFixed(1)}%
                      </span>
                    </div>
                    <div className="text-xs text-gray-500 mt-1">
                      {bestStrategy.recommendations.confidenceLevel === 'medium' && (
                        <span>Medium confidence: May need more diverse examples for better accuracy</span>
                      )}
                      {bestStrategy.recommendations.confidenceLevel === 'low' && (
                        <span>Low confidence: Limited test data or high variability detected</span>
                      )}
                      {bestStrategy.recommendations.confidenceLevel === 'high' && (
                        <span>High confidence: Strong pattern consistency across examples</span>
                      )}
                    </div>
                    <Button
                      size="sm"
          ...
          <TabsContent value="patterns" className="flex-1 space-y-2 overflow-y-auto p-2">
            <Card className="bg-[#27272A]/50 border-gray-700">
              <CardHeader className="pb-3">
                <CardTitle className="text-sm flex items-center justify-between">
                  <span>Extracted Patterns</span>
                  <span className="text-xs text-gray-400">
                    {selectedPatterns.size} / {analysis.extractedPatterns.length} selected
                  </span>
                </CardTitle>
              </CardHeader>
                  ...
                  <>
                    <div className="flex gap-2 mb-3">
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => setSelectedPatterns(new Set(analysis.extractedPatterns.map(p => p.id)))}
                          ...
                          >
                            ...
                            >
                              <div className="aspect-video bg-gray-800 rounded mb-2 flex items-center justify-center overflow-hidden">
                                {pattern.imageUrl ? (
                                  <img
                                    src={pattern.customMask ? pattern.customMask : pattern.imageUrl}
                                    alt={`Pattern ${idx + 1}`}
                              ...
                              <div className="flex items-center justify-between">
                                <span className="text-xs font-medium">Pattern {idx + 1}</span>
                                {isSelected && (
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-700">
                      <p className="text-xs text-gray-400 mb-2">
                        Select the patterns you want to include in the StateImage.
          ...
          <TabsContent value="strategies" className="flex-1 space-y-2 overflow-y-auto p-2">
            {/* Metrics explanation */}
                ...
                <div className="text-xs space-y-1">
                  <div className="mb-3 p-2 bg-[#00D9FF]/10 rounded">
                    <p className="text-xs text-[#00D9FF]">
                      <span className="font-medium">Workflow:</span> 1) Analyze patterns → 2) Select a strategy → 3) Create StateImages
                    </p>
                    <p className="text-xs text-gray-400 mt-1">
                      The "Select This Strategy" button chooses which method to use. The "Create StateImages" button adds them to your project.
                    </p>
                  </div>
                  <p className="font-medium text-gray-300 mb-2">Understanding Metrics:</p>
                  <div className="grid grid-cols-2 gap-2 text-gray-400">
                    <div>
                      <span className="font-medium text-green-400">TPR (True Positive Rate):</span>
                      <p className="mt-1">How often the strategy correctly identifies your pattern. Higher is better (aim for 90%+)</p>
                    </div>
                    <div>
                      <span className="font-medium text-red-400">FPR (False Positive Rate):</span>
                      <p className="mt-1">How often it incorrectly matches. Lower is better (aim for under 10%)</p>
                    </div>
                  </div>
                  <div className="mt-2 pt-2 border-t border-gray-700">
                    <p className="text-gray-500">
                      <span className="font-medium">Why medium confidence with 100% similarity?</span> Pattern similarity between your examples is perfect,
                      but the strategy needs more diverse test cases to prove it won't match incorrect patterns in real-world use.
                    </p>
                  </div>
                </div>
              </CardContent>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500 mt-1">
                          {type === 'multi-pattern' && 'Uses multiple pattern variations for robust matching'}
                          {type === 'consensus' && 'Finds common elements across all examples'}
                          {type === 'feature-based' && 'Analyzes visual features like edges and colors'}
                          {type === 'differential' && 'Identifies unique differences from negative examples'}
                        </p>

                        {evaluation ? (
                          <div className="mt-2 space-y-2">
                            <div className="flex items-center gap-4 text-xs">
                              <span className="flex items-center gap-1" title="True Positive Rate - Detection accuracy">
                                <CheckCircle className="w-3 h-3 text-green-500" />
                                <span className="text-gray-300">Accuracy:</span> {(evaluation.performance.truePositiveRate * 100).toFixed(1)}%
                              </span>
                              <span className="flex items-center gap-1" title="False Positive Rate - Mistake rate">
                                <AlertCircle className="w-3 h-3 text-red-500" />
                                <span className="text-gray-300">Errors:</span> {(evaluation.performance.falsePositiveRate * 100).toFixed(1)}%
                              </span>
                              <span className="flex items-center gap-1" title="Processing time per match">
                                <Activity className="w-3 h-3 text-[#00D9FF]" />
                                <span className="text-gray-300">Speed:</span> {evaluation.performance.processingTime.toFixed(0)}ms
                              </span>
                            </div>
                            <div>
                              <div className="flex justify-between items-center mb-1">
                                <span className="text-xs text-gray-500">Confidence</span>
                                <span className="text-xs text-gray-400">{(evaluation.performance.averageConfidence * 100).toFixed(0)}%</span>
                              </div>
                              <div className="h-1 bg-gray-700 rounded overflow-hidden">
                                <div
                                  className="h-full bg-[#00D9FF] transition-all"
                                  style={{ width: `${evaluation.performance.averageConfidence * 100}%` }}
                                />
                              </div>
                            </div>
                          </div>
          ...
          <TabsContent value="similarity" className="flex-1 overflow-y-auto p-2">
            <Card className="bg-[#27272A]/50 border-gray-700">
              <CardHeader className="pb-3">
                    ...
                    <thead>
                      <tr>
                        ...
                        {analysis.extractedPatterns.map((_, i) => (
                          <th key={i} className="p-1 text-center text-gray-400">P{i + 1}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
      ...
      {showStateImageDialog && analysis && selectedStrategy && (
        <StateImageCreationDialog
          ...
          patterns={analysis.extractedPatterns.filter(p => selectedPatterns.has(p.id))}
