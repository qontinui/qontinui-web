...
type ResizeHandle = 'nw' | 'n' | 'ne' | 'e' | 'se' | 's' | 'sw' | 'w' | 'inside' | null
...
export function ImageCanvas({
  ...
  const findBoxAtPoint = useCallback(
    (x: number, y: number): { box: BoundingBox; handle: ResizeHandle } | null => {
    ...
    allBoxes.forEach((box) => {
      ...
      if (isSelected && !readonly) {
        ...
        const handles = [
          ...
          { x: screenPos.x + screenSize.width, y: screenPos.y + screenSize.height / 2 }, // e
          { x: screenPos.x + screenSize.width, y: screenPos.y + screenSize.height }, // se
          { x: screenPos.x + screenSize.width / 2, y: screenPos.y + screenSize.height }, // s
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (isDragging && dragStart && selectedBoxId) {
        ...
        if (imageRef.current) {
          updatedBox.x = Math.max(0, Math.min(imageRef.current.width - updatedBox.width, updatedBox.x))
          updatedBox.y = Math.max(0, Math.min(imageRef.current.height - updatedBox.height, updatedBox.y))
        }

        const updatedBoxes = boxes.map((b) => (b.id === selectedBoxId ? updatedBox : b))
  ...
  const handleMouseUp = useCallback(() => {
    ...
    setDragStart(null)
    setDragHandle(null)
  }, [isPanning, isDrawing, isDragging, tempBox, boxes, minBoxSize, onBoxesChange, onBoxSelect])
