...
interface OptimizationSuggestion {
  id: string;
  type: 'parallelization' | 'wait-optimization' | 'loop-optimization' | 'caching' | 'action-removal';
  severity: 'low' | 'medium' | 'high';
  title: string;
  description: string;
...
function generateMockPerformanceData(workflow: Workflow): PerformanceData {
  const actionTimings: ActionTiming[] = workflow.actions.map((action, index) => ({
    actionId: action.id,
  ...
  const bottlenecks: Bottleneck[] = actionTimings
    ...
    .map(t => ({
      ...
      severity: t.duration > totalDuration * 0.3 ? 'high' : t.duration > totalDuration * 0.2 ? 'medium' : 'low',
    }));
  ...
  const suggestions: OptimizationSuggestion[] = [
    {
      ...
      description: 'Actions 3, 4, and 5 can be executed in parallel as they have no dependencies.',
    ...
    },
    {
      ...
      description: 'WAIT action is using a fixed 5s delay. Consider using a dynamic wait with timeout.',
      estimatedImprovement: 3000,
      impactPercentage: 30,
      affectedActions: workflow.actions.filter(a => a.type === 'WAIT').map(a => a.id),
    },
    {
      id: 'opt-3',
      ...
      description: 'Loop is executing 50 times. Consider batching operations or reducing iterations.',
      estimatedImprovement: 1500,
      impactPercentage: 15,
      affectedActions: workflow.actions.filter(a => a.type === 'LOOP').map(a => a.id),
    },
    {
      id: 'opt-4',
      type: 'caching',
      ...
      description: 'Multiple FIND actions search for the same pattern. Cache results to avoid redundant searches.',
      estimatedImprovement: 800,
      impactPercentage: 8,
      affectedActions: workflow.actions.filter(a => a.type === 'FIND').map(a => a.id),
    },
  ];
  ...
  const estimatedOptimizedDuration = totalDuration - suggestions.reduce((sum, s) => sum + s.estimatedImprovement, 0);
      ...
      <CardContent className="space-y-3">
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <div className="text-right">
            <div className="text-2xl font-bold">{formatPercentage(suggestion.impactPercentage)}</div>
            <div className="text-xs text-muted-foreground">of total time</div>
          </div>
        </div>

        <div className="text-xs text-muted-foreground">
          Affects {suggestion.affectedActions.length} action{suggestion.affectedActions.length !== 1 ? 's' : ''}
        </div>
...
export function PerformanceAnalyzer({
  ...
  const activeSuggestions = useMemo(() => {
    return performanceData.suggestions.filter(s => !dismissedSuggestions.has(s.id));
  }, [performanceData.suggestions, dismissedSuggestions]);

  // Calculate potential savings
  const totalPotentialSavings = useMemo(() => {
    return activeSuggestions.reduce((sum, s) => sum + s.estimatedImprovement, 0);
  }, [activeSuggestions]);
  ...
  const heatmapData = useMemo(() => {
    return performanceData.actionTimings.map(t => ({
      ...
      duration: t.duration,
      fill: t.duration > performanceData.totalDuration * 0.2 ? '#ef4444' :
            t.duration > performanceData.totalDuration * 0.1 ? '#f59e0b' : '#10b981',
    }));
  }, [performanceData]);
    ...
    <div className={cn('space-y-6', className)}>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium">Current Duration</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{formatDuration(performanceData.totalDuration)}</div>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium">Optimized Duration</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium">Bottlenecks Found</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{performanceData.bottlenecks.length}</div>
          </CardContent>
        </Card>
      </div>
      ...
      {totalPotentialSavings > 0 && (
        <Alert>
          ...
          <AlertDescription>
            You can save up to <strong>{formatDuration(totalPotentialSavings)}</strong> (
            {formatPercentage((totalPotentialSavings / performanceData.totalDuration) * 100)}) by applying
            ...
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-12">
                ...
                <p className="text-muted-foreground text-center">
                  This workflow is already well-optimized, or run an analysis to find improvements.
                </p>
              </CardContent>
            </Card>
          )}
        ...
        <TabsContent value="bottlenecks" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Actions consuming the most time (red = high, yellow = medium, green = low)
              </CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Actions taking the most execution time</CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <div className="space-y-3">
                  {performanceData.bottlenecks.map((bottleneck, index) => (
                    <div key={bottleneck.actionId} className="space-y-2">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-medium">{bottleneck.actionName}</span>
                            <Badge variant="outline" className="text-xs">
                              {bottleneck.actionType}
                            </Badge>
                          </div>
                          <div className="text-xs text-muted-foreground mt-1">
                            {formatPercentage(bottleneck.percentOfTotal)} of total execution time
                          </div>
                        </div>
                        <div className="text-right">
                          <div className="text-sm font-bold">{formatDuration(bottleneck.duration)}</div>
                          <Badge
                            variant="outline"
                      ...
                      />
                      {index < performanceData.bottlenecks.length - 1 && <Separator className="mt-3" />}
                    </div>
                  ))}
                </div>
              </ScrollArea>
              ...
              {performanceData.parallelizationOpportunities.length > 0 ? (
                <div className="space-y-4">
                  {performanceData.parallelizationOpportunities.map((opp) => (
                    <div key={opp.groupId} className="border rounded-lg p-4 space-y-3">
                      <div className="flex items-center justify-between">
                        <h4 className="font-semibold">Group {opp.groupId}</h4>
                        <Badge variant="secondary">{opp.actions.length} actions</Badge>
                      </div>

                      <div className="grid grid-cols-3 gap-4 text-sm">
                        <div>
                          <div className="text-muted-foreground">Current Duration</div>
                          <div className="font-bold">{formatDuration(opp.currentDuration)}</div>
                        </div>
                        <div>
                          <div className="text-muted-foreground">Parallel Duration</div>
                          <div className="font-bold text-green-500">{formatDuration(opp.parallelDuration)}</div>
                        </div>
                        <div>
                          <div className="text-muted-foreground">Time Saved</div>
                          <div className="font-bold text-blue-500">{formatDuration(opp.savings)}</div>
                        </div>
                      </div>
              ...
              {performanceData.waitAnalysis.length > 0 ? (
                <div className="space-y-3">
                  {performanceData.waitAnalysis.map((wait) => (
                    <div key={wait.actionId} className="border rounded-lg p-4 space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="font-medium">{wait.actionName}</span>
                        <Badge variant="outline">{formatDuration(wait.waitDuration)}</Badge>
                      </div>

                      <p className="text-sm text-muted-foreground">{wait.suggestion}</p>

                      <div className="flex items-center justify-between text-sm">
                        <span className="text-muted-foreground">Potential savings:</span>
                        <span className="font-bold text-green-500">{formatDuration(wait.potentialSavings)}</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {performanceData.loopAnalysis.length > 0 ? (
                <div className="space-y-3">
                  {performanceData.loopAnalysis.map((loop) => (
                    <div key={loop.actionId} className="border rounded-lg p-4 space-y-3">
                      <div className="flex items-center justify-between">
                        <span className="font-medium">{loop.actionName}</span>
                        <Badge variant="secondary">{loop.estimatedIterations} iterations</Badge>
                      </div>

                      <div className="grid grid-cols-2 gap-3 text-sm">
                        <div>
                          <div className="text-muted-foreground">Avg Iteration</div>
                          <div className="font-bold">{formatDuration(loop.avgIterationDuration)}</div>
                        </div>
                        <div>
                          <div className="text-muted-foreground">Total Duration</div>
                          <div className="font-bold">{formatDuration(loop.totalDuration)}</div>
                        </div>
                      </div>

                      <p className="text-sm text-muted-foreground">{loop.suggestion}</p>

                      <div className="flex items-center justify-between text-sm">
                        <span className="text-muted-foreground">Potential savings:</span>
                        <span className="font-bold text-green-500">{formatDuration(loop.potentialSavings)}</span>
                      </div>
                    </div>
                  ))}
                </div>
