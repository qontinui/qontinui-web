...
export const MaskEditor: React.FC<MaskEditorProps> = ({
  ...
  const imageDimensionsRef = useRef<{ width: number; height: number }>({ width: 27, height: 27 })
  ...
  const initializeCanvases = (img: HTMLImageElement) => {
    ...
    if (initialMask) {
      ...
      maskImg.onload = () => {
        ...
        const loadedMaskData = maskCtx.getImageData(0, 0, canvasWidth, canvasHeight)
  ...
  const findMaskBounds = () => {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        if (maskValue > 0) { // White pixel (visible)
          foundWhite = true
  ...
  const handleSave = () => {
    ...
    maskCtx.drawImage(fullMaskCanvas, cropX, cropY, cropWidth, cropHeight, 0, 0, cropWidth, cropHeight)
    ...
    ctx.drawImage(originalCanvasRef.current, cropX, cropY, cropWidth, cropHeight, 0, 0, cropWidth, cropHeight)
    ...
    toast.success(cropToMask && bounds ? `Mask saved (cropped to ${cropWidth}x${cropHeight})` : 'Mask saved')
  }
  ...
  const imageName = imageNameProp || imageUrl.split('/').pop()?.split('?')[0] || 'Image'
    ...
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onCancel()}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-3 border-b border-gray-800 shrink-0">
          <DialogTitle className="text-lg font-semibold">Mask Editor - {imageName}</DialogTitle>
          <DialogDescription className="sr-only">
            Edit mask by painting transparent areas with brush and revealing areas with eraser
          </DialogDescription>
          <div className="flex items-center gap-3 mr-8">
            ...
            <Button onClick={handleSave} size="sm" className="bg-[#BD00FF] hover:bg-[#BD00FF]/80 text-white">
        ...
        <div className="flex flex-1 overflow-hidden min-h-0">
          ...
          <div className="w-12 bg-gray-800 flex flex-col items-center py-4 gap-2">
            <Button
              variant="ghost"
              size="icon"
              className={cn("w-9 h-9", tool === "brush" && "bg-[#BD00FF] hover:bg-[#BD00FF]/80 text-white")}
            ...
            <Button
              variant="ghost"
              size="icon"
              className={cn("w-9 h-9", tool === "eraser" && "bg-[#BD00FF] hover:bg-[#BD00FF]/80 text-white")}
            ...
            <Button variant="ghost" size="icon" className="w-9 h-9" onClick={undo} disabled={historyIndex <= 0} title="Undo">
          ...
          <div className="flex-1 flex flex-col min-w-0">
            ...
            <div className="flex-1 flex gap-4 p-4 bg-[#0A0A0B] min-h-0">
              ...
              <div className="flex-1 flex flex-col gap-2 min-w-0">
                ...
                <div ref={resultContainerRef} className="flex-1 bg-gray-800 rounded overflow-hidden relative min-h-0">
                  <canvas
                    ref={resultCanvasRef}
              ...
              <div className="w-64 flex flex-col gap-4 shrink-0">
                ...
                <div className="flex-1 flex flex-col gap-2 min-h-0">
                  <h3 className="text-sm font-medium text-gray-400 text-center shrink-0">Original Image</h3>
                ...
                {/* Mask Visualization */}
                <div className="flex-1 flex flex-col gap-2 min-h-0">
                  <h3 className="text-sm font-medium text-gray-400 text-center shrink-0">Mask</h3>
                  <div className="flex-1 bg-gray-800 rounded overflow-hidden min-h-0">
                    <canvas
                      ref={maskCanvasRef}
                      className="w-full h-full object-contain"
            ...
            <div className="bg-gray-800 p-2 text-xs text-gray-400 shrink-0">
              <ul className="list-disc list-inside space-y-1">
                <li>
                  <strong className="text-gray-300">Result:</strong> Draw on this canvas to edit the mask (shows final
                  image with transparency)
                </li>
                <li>
                  <strong className="text-gray-300">Original:</strong> Unmodified source image
                </li>
                <li>
                  <strong className="text-gray-300">Mask:</strong> Visual representation of the mask (white = visible,
                  black = transparent)
                </li>
                <li>
                  <strong className="text-gray-300">Brush:</strong> Add pixels to mask (make areas visible)
                </li>
                <li>
                  <strong className="text-gray-300">Eraser:</strong> Remove pixels from mask (make areas transparent)
                </li>
                <li>
