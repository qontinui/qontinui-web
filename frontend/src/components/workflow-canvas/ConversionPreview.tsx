...
export function ConversionPreview({
  ...
  const [activeTab, setActiveTab] = useState<'visual' | 'changes' | 'warnings'>('visual');
    ...
    <div className="conversion-preview">
              ...
              <div className="conversion-statistics">
                ...
                <div className="stats-grid">
                  <div className="stat-card">
                    <div className="stat-value added">{conversionPreview.changes.actionsAdded}</div>
                    <div className="stat-label">Actions Added</div>
                  </div>
                  <div className="stat-card">
                    <div className="stat-value removed">{conversionPreview.changes.actionsRemoved}</div>
                    <div className="stat-label">Actions Removed</div>
                  </div>
                  <div className="stat-card">
                    <div className="stat-value modified">{conversionPreview.changes.actionsModified}</div>
                    <div className="stat-label">Actions Modified</div>
                  </div>
                  <div className="stat-card">
                    <div className="stat-value changed">{conversionPreview.changes.connectionsChanged}</div>
        ...
        {activeTab === 'changes' && (
          <div className="changes-tab">
            <div className="changes-summary">
              ...
              <div className="summary-item">
                <span className="badge modified">~{modifiedActions.length}</span>
                <span>Modified</span>
              </div>
            </div>
            ...
            {conversionPreview.changes.connectionsChanged > 0 && (
              <div className="connection-changes">
                ...
                <p>
                  ...
                  {conversionPreview.changes.connectionsChanged > 1 ? 's' : ''} will be
                  {conversionPreview.fromFormat === 'sequential' ? ' created' : ' removed/modified'}
                </p>
                {conversionPreview.fromFormat === 'sequential' && (
                  <p className="info-text">
                    Sequential actions will be connected with explicit edges in the graph format.
                  </p>
                )}
          ...
          <div className="warnings-tab">
            ...
            {conversionPreview.linearizability && (
              <div className="linearizability-info">
                ...
                <div className={`linearizable-status ${conversionPreview.linearizability.linearizable ? 'success' : 'error'}`}>
                ...
                {conversionPreview.linearizability.details && (
                  <div className="linearizability-details">
                    <div className="detail-row">
                      ...
                      <span>{conversionPreview.linearizability.details.entryPointCount}</span>
                    </div>
                    <div className="detail-row">
                      <span>Branch Points:</span>
                      <span>{conversionPreview.linearizability.details.branchingNodeCount}</span>
                    </div>
                    <div className="detail-row">
                      <span>Merge Points:</span>
                      <span>{conversionPreview.linearizability.details.mergeNodeCount}</span>
                    </div>
                    <div className="detail-row">
                      <span>Parallel Branches:</span>
                      <span>{conversionPreview.linearizability.details.parallelBranchCount}</span>
                    </div>
                    <div className="detail-row">
                      <span>Cycles:</span>
                      <span>{conversionPreview.linearizability.details.cycleCount}</span>
                    </div>
                  </div>
                )}
      ...
      <div className={`impact-banner impact-${conversionPreview.impact}`}>
        ...
        <div className="recommendation-label">
          <strong>Recommendation:</strong> {conversionPreview.recommendation.replace('_', ' ').toUpperCase()}
        </div>
      </div>
    </div>
  );
    ...
    <div className={`change-item change-${change.type}`}>
      ...
      <div className="change-content">
        <div className="change-header">
          ...
          {change.actionName && <span className="action-name">{change.actionName}</span>}
        </div>
        {change.details && (
...
function analyzeChanges(before: Workflow, after: Workflow): ChangeType[] {
  ...
  for (const beforeAction of before.actions) {
    ...
    if (afterAction) {
      ...
      const configChanged = JSON.stringify(beforeAction.config) !== JSON.stringify(afterAction.config);
      const positionChanged = beforeAction.position && afterAction.position &&
