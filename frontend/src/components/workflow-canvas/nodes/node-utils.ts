...
export function getNodeCategory(actionType: ActionType): NodeCategory {
  // Find actions
  if (['FIND', 'FIND_STATE_IMAGE', 'VANISH', 'EXISTS', 'WAIT'].includes(actionType)) {
    return 'find';
  }
  ...
  // Keyboard actions
  if (['TYPE', 'KEY_PRESS', 'KEY_DOWN', 'KEY_UP', 'HOTKEY'].includes(actionType)) {
    return 'keyboard';
  }

  // Control flow actions
  if (['IF', 'LOOP', 'BREAK', 'CONTINUE', 'SWITCH', 'TRY_CATCH'].includes(actionType)) {
    return 'controlFlow';
  }
 ...
 */
export function getHandlePositions(outputCount: number): Array<{ top: string }> {
