...
import { Lightbulb, X, Plus, Info, Zap, Shield, AlertTriangle, TrendingUp, CheckCircle } from 'lucide-react';
...
export function AISuggestions({
  ...
  const handleApply = useCallback((suggestion: WorkflowSuggestion) => {
  ...
  const sortedTypes = Object.keys(groupedSuggestions).sort((a, b) => {
    ...
    const aImpact = Math.min(...groupedSuggestions[a].map(s => impactOrder[s.impact]));
    const bImpact = Math.min(...groupedSuggestions[b].map(s => impactOrder[s.impact]));
                      ...
                      >
                        <div className="p-3">
                          <div className="flex items-start gap-3">
                            <div className={`p-2 rounded-lg ${SUGGESTION_COLORS[suggestion.type]}`}>
