...
export function KeyboardShortcuts({ onClose }: KeyboardShortcutsProps) {
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-surface-raised border border-border-default rounded-lg shadow-2xl w-full max-w-3xl max-h-[80vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-border-default">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handlePrint}
              className="px-3 py-2 bg-surface-raised hover:bg-surface-raised/80 text-white rounded text-sm transition-colors"
        ...
        <div className="p-4 border-b border-border-default">
          <div className="relative">
            <input
              ...
              className="w-full px-4 py-2 pl-10 bg-surface-canvas border border-border-default rounded-lg text-white placeholder-text-muted focus:outline-none focus:border-brand-primary"
            />
            <svg
              ...
              <div className="space-y-2">
                {filteredShortcuts
                  ...
                  .map((shortcut) => (
                    <div
                      key={shortcut.id}
                      className="flex items-center justify-between p-3 bg-surface-canvas rounded-lg"
                    >
                      ...
                      <div className="flex items-center gap-1">
                        {shortcut.keys.map((key, index) => (
                          <React.Fragment key={index}>
                            ...
                            <kbd className="px-2 py-1 text-xs font-mono bg-surface-raised border border-border-default rounded text-text-secondary">
                              {key}
                            </kbd>
                          </React.Fragment>
                        ))}
        ...
        {/* Footer */}
        <div className="p-4 border-t border-border-default text-sm text-text-muted">
          <p>
            <strong className="text-white">Tip:</strong> Press{" "}
            <kbd className="px-2 py-1 bg-surface-canvas border border-border-default rounded">
              ?
            </kbd>{" "}
