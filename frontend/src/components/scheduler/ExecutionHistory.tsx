...
export function ExecutionHistory() {
  ...
  const filteredRecords = executionRecords.filter(record => {
    if (selectedScheduleFilter !== "all" && record.scheduleId !== selectedScheduleFilter) {
      return false
    }
            ...
            <Card>
              <CardContent className="pt-6 text-center text-muted-foreground">
                ...
                <p className="text-sm mt-2">
                  {selectedScheduleFilter !== "all" || selectedStatusFilter !== "all"
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div>
                      ...
                      <p className="font-medium">{record.startTime.toLocaleString()}</p>
                    </div>
                    <div>
                    ...
                    {record.endTime && (
                      <div>
                        ...
                        <p className="font-medium">{record.endTime.toLocaleString()}</p>
                      </div>
                    )}
                  </div>
                    ...
                    <div className="mt-3 pt-3 border-t">
                      ...
                      <div className="space-y-1">
                        {record.errors.map((error, index) => (
                          <p key={index} className="text-sm text-muted-foreground bg-red-50 dark:bg-red-950 p-2 rounded">
                            {error}
                          </p>
                        ))}
                      </div>
