...
describe('Workflow Utilities', () => {
  describe('Entry Points', () => {
    test('should return first action for sequential workflow', () => {
      const workflow: Workflow = {
        ...
        version: '1.0.0',
        actions: [
          createAction('CLICK', { target: { image: 'test.png' } }, { id: 'action-1' }),
    ...
    test('should return actions with no incoming connections for graph workflow', () => {
      ...
      expect(getEntryPoints(workflow).sort()).toEqual(['action-1', 'action-3'].sort());
    });
  });
  ...
  describe('Workflow Cloning', () => {
    test('should clone workflow with new IDs', () => {
      ...
      expect(cloned.connections![firstActionId].main![0][0].action).toBe(secondActionId);
...
describe('Workflow Validation', () => {
  describe('Basic Validation', () => {
    ...
    test('should detect duplicate action IDs', () => {
      const workflow: Workflow = {
        ...
        version: '1.0.0',
        actions: [
          createAction('CLICK', { target: { image: 'test.png' } }, { id: 'duplicate' }),
          createAction('TYPE', { text: 'hello' }, { id: 'duplicate' }),
        ],
      };
      const result = validateWorkflow(workflow);
      expect(result.valid).toBe(false);
      expect(result.errors.some((e) => e.type === 'duplicate_action_id')).toBe(true);
    });
  });
  ...
  describe('Connection Validation', () => {
    ...
    test('should detect invalid output index', () => {
      ...
      const result = validateConnections(workflow);
      expect(result.valid).toBe(false);
      expect(result.errors.some((e) => e.type === 'invalid_output_index')).toBe(true);
    });
  ...
  describe('Orphan Validation', () => {
    test('should warn about orphaned actions', () => {
      ...
      expect(result.warnings.length).toBeGreaterThan(0);
      expect(result.warnings.some((w) => w.actionId === 'action-orphan')).toBe(true);
    });
  });
  ...
  describe('Format-Specific Validation', () => {
    test('should error on connections in sequential workflow', () => {
      ...
      const result = validateWorkflow(workflow);
      expect(result.valid).toBe(false);
      expect(result.errors.some((e) => e.type === 'connections_in_sequential')).toBe(true);
    });
    ...
    test('should error on graph workflow without connections', () => {
      ...
      const result = validateWorkflow(workflow);
      expect(result.valid).toBe(false);
      expect(result.errors.some((e) => e.type === 'missing_connections_in_graph')).toBe(true);
    });
  });
...
describe('Backward Compatibility', () => {
  ...
  test('should work with actions without position field', () => {
    const action = createAction('CLICK', { target: { image: 'test.png' } }, { id: 'action-1' });
  ...
  test('should handle workflows without connections', () => {
    const workflow: Workflow = {
      ...
      name: 'Test',
      version: '1.0.0',
      actions: [createAction('CLICK', { target: { image: 'test.png' } }, { id: 'action-1' })],
...
describe('Complex Graph Scenarios', () => {
  test('should handle IF action with branching', () => {
    const workflow: Workflow = {
      ...
      actions: [
        ...
        createAction('CLICK', { target: { image: 'test.png' } }, { id: 'then-action' }),
    ...
    expect(getActionOutputCount('IF')).toBe(2);
    expect(getNextActions(workflow, 'if-1')).toEqual(['then-action', 'else-action']);
    expect(isWorkflowValid(workflow)).toBe(true);
  });
  ...
  test('should handle parallel execution branches', () => {
    const workflow: Workflow = {
      ...
      format: 'graph',
      actions: [
        createAction('CLICK', { target: { image: 'start.png' } }, { id: 'start' }),
    ...
    expect(getNextActions(workflow, 'start', 'parallel')).toEqual(['parallel-a', 'parallel-b']);
    expect(isWorkflowValid(workflow)).toBe(true);
  });
  ...
  test('should handle TRY_CATCH with error paths', () => {
    const workflow: Workflow = {
      ...
      actions: [
        ...
        createAction('CLICK', { target: { image: 'success.png' } }, { id: 'success' }),
        createAction('CLICK', { target: { image: 'error.png' } }, { id: 'error' }),
      ],
      connections: {
        'try-1': {
