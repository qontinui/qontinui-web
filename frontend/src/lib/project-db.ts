...
class ProjectDB {
  ...
  private async getByProject<T>(
    ...
    try {
      ...
      console.error(
        `Error getting ${storeName} for project ${projectName}:`,
        error
      );
      return [];
    }
  ...
  private async add<T extends { id: string }>(
    storeName: string,
    item: T
  ): Promise<void> {
    try {
  ...
  async addWorkflow(
    workflow: Workflow & { projectName: string }
  ): Promise<void> {
  ...
  async renameProject(
    ...
    await Promise.all([
      ...
      ...states.map((s) =>
        this.updateState({ ...s, projectName: newProjectName })
      ),
      ...transitions.map((t) =>
        this.updateTransition({ ...t, projectName: newProjectName })
      ),
      ...images.map((i) =>
        this.updateImage({ ...i, projectName: newProjectName })
      ),
    ]);
  }
}
