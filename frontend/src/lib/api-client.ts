...
export interface BackoffConfig {
  ...
  multiplier: number;
  // null = unbounded (no cap on the delay between retries)
...
export interface AutoResponseRuleCreate {
  name: string;
  pattern: string;
  prompt: string;
  enabled?: boolean;
...
export interface AutoResponseRuleUpdate {
  name?: string;
  pattern?: string;
  prompt?: string;
  enabled?: boolean;
...
class ApiClient {
  ...
  // ===== Auto-Response Rule Endpoints =====

  async getAutoResponseRules(orgId: string): Promise<AutoResponseRule[]> {
    const response = await this.fetchWithAuth(
      `/organizations/${orgId}/auto-response-rules`
    );
    if (!response.ok) {
      throw new Error("Failed to get auto-response rules");
    }
    const data = await response.json();
    return data.items;
  }

  async createAutoResponseRule(
    orgId: string,
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(
        errorData.detail || "Failed to create auto-response rule"
      );
    }
    return response.json();
  }

  async updateAutoResponseRule(
    orgId: string,
    id: string,
    data: AutoResponseRuleUpdate
  ): Promise<AutoResponseRule> {
    const response = await this.fetchWithAuth(
      `/organizations/${orgId}/auto-response-rules/${id}`,
      {
        method: "PUT",
        body: JSON.stringify(data),
      }
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(
        errorData.detail || "Failed to update auto-response rule"
      );
    }
    return response.json();
  }

  async deleteAutoResponseRule(orgId: string, id: string): Promise<void> {
    const response = await this.fetchWithAuth(
      `/organizations/${orgId}/auto-response-rules/${id}`,
      {
        method: "DELETE",
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(
        errorData.detail || "Failed to delete auto-response rule"
      );
    }
  }

  async reorderAutoResponseRules(
    orgId: string,
    ids: string[]
  ): Promise<AutoResponseRule[]> {
    const response = await this.fetchWithAuth(
      `/organizations/${orgId}/auto-response-rules/reorder`,
      {
        method: "POST",
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(
        errorData.detail || "Failed to reorder auto-response rules"
      );
    }
    const data = await response.json();
    return data.items;
