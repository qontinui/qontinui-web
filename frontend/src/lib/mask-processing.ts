...
export async function applyBackgroundRemoval(
  ...
  try {
    ...
    const combinedMaskData = new ImageData(currentMaskData.width, currentMaskData.height)
...
export async function applyBorderRemoval(
  ...
  try {
    ...
    // Get edge color
    const edgeColor = getEdgeColor(imageData, region.x, region.y, region.width, region.height)
    ...
    for (let y = 0; y < canvas.height; y++) {
      for (let x = 0; x < canvas.width; x++) {
        ...
        const isBorder = colorMatches(r, g, b, edgeColor.r, edgeColor.g, edgeColor.b, tolerance)
