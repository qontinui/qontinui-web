...
export interface DeficiencyActivity {
  ...
  user_id: string;
  user_name: string;
  action: 'created' | 'updated' | 'status_changed' | 'assigned' | 'commented' | 'resolved';
  details: {
    field?: string;
    old_value?: any;
    new_value?: any;
...
export const WORKFLOW_TRANSITIONS: Record<DeficiencyStatus, DeficiencyStatus[]> = {
  [DeficiencyStatus.NEW]: [DeficiencyStatus.ACKNOWLEDGED, DeficiencyStatus.WONT_FIX],
  [DeficiencyStatus.ACKNOWLEDGED]: [DeficiencyStatus.INVESTIGATING, DeficiencyStatus.WONT_FIX],
  [DeficiencyStatus.INVESTIGATING]: [DeficiencyStatus.FIXED, DeficiencyStatus.WONT_FIX],
  [DeficiencyStatus.FIXED]: [DeficiencyStatus.CLOSED, DeficiencyStatus.INVESTIGATING],
  [DeficiencyStatus.CLOSED]: [DeficiencyStatus.INVESTIGATING],
  [DeficiencyStatus.WONT_FIX]: [DeficiencyStatus.INVESTIGATING]
};

// Status display configuration
export const STATUS_CONFIG: Record<DeficiencyStatus, {
  label: string;
  color: string;
  bgColor: string;
  icon: string;
...
export const SEVERITY_CONFIG: Record<DeficiencySeverity, {
  label: string;
  color: string;
  bgColor: string;
}> = {
