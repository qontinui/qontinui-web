...
function createLinearWorkflow(): Workflow {
  const click1 = createAction('CLICK', { target: { image: 'button.png' } }, [100, 100], {
    id: 'action-1',
    name: 'Click Button',
  });

  const wait = createAction('WAIT', { duration: 1000 }, [100, 250], {
    id: 'action-2',
    name: 'Wait 1 Second',
  });

  const click2 = createAction('CLICK', { target: { image: 'submit.png' } }, [100, 400], {
    id: 'action-3',
    name: 'Click Submit',
  });
...
function createConditionalWorkflow(): Workflow {
  const find = createAction(
    'EXISTS',
    { target: { image: 'element.png' }, timeout: 5000 },
    [100, 100],
  ...
  const ifAction = createAction(
    'IF',
    { condition: { type: 'found', variable: 'check' }, thenActions: [], elseActions: [] },
    [100, 250],
    { id: 'if-1', name: 'If Found' }
  );

  const thenAction = createAction('CLICK', { target: { image: 'element.png' } }, [300, 400], {
    id: 'then-1',
    name: 'Click Element',
  });
  ...
  const elseAction = createAction(
    'TYPE',
    { text: 'Element not found', target: { selector: 'input' } },
    [500, 400],
...
function createLoopWorkflow(): Workflow {
  const loop = createAction(
    'LOOP',
    { iterations: 5, loopActions: [], variable: 'i' },
    [100, 100],
    { id: 'loop-1', name: 'Repeat 5 Times' }
  );

  const click = createAction('CLICK', { target: { image: 'button.png' } }, [300, 100], {
    id: 'loop-action',
    name: 'Click Button',
  });

  const wait = createAction('WAIT', { duration: 500 }, [300, 250], {
    id: 'loop-wait',
    name: 'Wait 500ms',
  });

  const done = createAction('TYPE', { text: 'Done', target: { selector: 'input' } }, [100, 400], {
    id: 'done',
    name: 'Type Done',
  });
...
function createErrorHandlingWorkflow(): Workflow {
  ...
  const riskyAction = createAction(
    'CLICK',
    { target: { image: 'button.png' }, timeout: 2000 },
    [300, 100],
    { id: 'risky', name: 'Click (May Fail)' }
  );

  const success = createAction('TYPE', { text: 'Success', target: { selector: 'input' } }, [500, 100], {
    id: 'success',
    name: 'Success Handler',
  });

  const error = createAction('SCREENSHOT', { filename: 'error.png' }, [500, 250], {
    id: 'error',
...
function createDataProcessingWorkflow(): Workflow {
  ...
  const map = createAction(
    'MAP',
    { transformation: { field: 'name', operation: 'uppercase' } },
    [100, 400],
    { id: 'map', name: 'Map to Uppercase' }
  );

  const reduce = createAction('REDUCE', { operation: 'count' }, [100, 550], {
    id: 'reduce',
    name: 'Count Items',
  });
  ...
  const save = createAction(
    'SET_VARIABLE',
    { variable: 'result', value: '${reduce.output}', scope: 'local' },
    [100, 700],
  ...
  return {
    ...
    metadata: {
      created: new Date().toISOString(),
      description: 'Data processing pipeline with filter, map, and reduce operations',
    },
  };
}
...
function createFormAutomationWorkflow(): Workflow {
  const clickName = createAction('CLICK', { target: { selector: '#name' } }, [100, 100], {
    id: 'click-name',
    name: 'Click Name Field',
  });

  const typeName = createAction('TYPE', { text: 'John Doe', target: { selector: '#name' } }, [100, 250], {
    id: 'type-name',
    name: 'Type Name',
  });

  const clickEmail = createAction('CLICK', { target: { selector: '#email' } }, [100, 400], {
    id: 'click-email',
  ...
  const typeEmail = createAction(
    'TYPE',
    { text: 'john@example.com', target: { selector: '#email' } },
    [100, 550],
    { id: 'type-email', name: 'Type Email' }
  );

  const submit = createAction('CLICK', { target: { selector: 'button[type="submit"]' } }, [100, 700], {
    id: 'submit',
    name: 'Click Submit',
  });
...
function createScrapingWorkflow(): Workflow {
  const find = createAction('FIND', { target: { selector: '.product-card' } }, [100, 100], {
    id: 'find-products',
  ...
  const extract = createAction(
    'GET_VARIABLE',
    { variable: 'found.text', scope: 'local' },
    [100, 250],
  ...
  const filter = createAction(
    'FILTER',
    { condition: { field: 'price', operator: 'lessThan', value: 100 } },
    [100, 400],
  ...
  const save = createAction(
    'SET_VARIABLE',
    { variable: 'products', value: '${filter.output}', scope: 'process' },
    [100, 550],
    { id: 'save-results', name: 'Save Results' }
  );

  const screenshot = createAction('SCREENSHOT', { filename: 'products.png' }, [100, 700], {
    id: 'screenshot',
...
export class WorkflowTemplatesService {
  ...
  getTemplate(id: string): WorkflowTemplate | undefined {
    return [...BUILTIN_TEMPLATES, ...this.customTemplates].find((t) => t.id === id);
  }
  ...
  getCategories(): TemplateCategory[] {
    return ['basic', 'control-flow', 'data-processing', 'automation', 'advanced', 'custom'];
  }
  ...
  private saveCustomTemplates(): void {
    try {
      localStorage.setItem('workflow-custom-templates', JSON.stringify(this.customTemplates));
    } catch (error) {
