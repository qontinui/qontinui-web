...
export class AnalyticsService {
  ...
  async getUsageSummary(): Promise<UsageSummary> {
    const response = await this.httpClient.fetch(`${this.apiUrl}/api/v1/analytics/usage`);
    if (!response.ok) {
  ...
  async getMetrics(days?: number): Promise<MetricData[]> {
    ...
    const response = await this.httpClient.fetch(`${this.apiUrl}/api/v1/analytics/metrics${queryParam}`);
    if (!response.ok) {
  ...
  async getAnalyticsSummary(days?: number): Promise<AnalyticsSummary> {
    const queryParam = days ? `?days=${days}` : '';
    const response = await this.httpClient.fetch(`${this.apiUrl}/api/v1/analytics/summary${queryParam}`);
    if (!response.ok) {
  ...
  async getStorageBreakdown(): Promise<StorageBreakdown> {
    const response = await this.httpClient.fetch(`${this.apiUrl}/api/v1/analytics/storage-breakdown`);
    if (!response.ok) {
  ...
  async getActivityTimeline(limit?: number): Promise<ActivityItem[]> {
    ...
    const response = await this.httpClient.fetch(`${this.apiUrl}/api/v1/analytics/activity${queryParam}`);
    if (!response.ok) {
