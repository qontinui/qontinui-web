...
export function example2_BuildDependencyGraph() {
  ...
  for (const [, node] of graph.nodes) {
    ...
    console.log(
      `    - Dependents (${node.inDegree}): ${node.dependents.join(", ") || "none"}`
    );
...
export function example6_DependencyStatistics() {
  ...
  console.log(
    `Avg Dependencies per Workflow: ${stats.avgDependenciesPerWorkflow.toFixed(2)}`
  );
  console.log(
    `Avg Dependents per Workflow: ${stats.avgDependentsPerWorkflow.toFixed(2)}`
  );
...
export function example9_ExportReport() {
  ...
  console.log(
    `  Circular Dependencies: ${report.statistics.circularDependencies}`
  );
