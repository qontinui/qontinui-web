...
```typescript
const { conflicts, autoResolve } = useConflictResolution(
  projectId,
  "workflow",
  workflowId,
  {
...
const { applyOptimistic, rollback } = useOptimisticUpdate(
  "workflow",
  workflowId
);
...
const { isConnected, remoteChanges } = useRealtimeCollaboration(
  projectId,
  "workflow",
  workflowId
);
