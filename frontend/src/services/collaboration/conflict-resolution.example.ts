...
export async function basicConflictResolution() {
  ...
  const localVersion = {
    ...
    connections: [
      { id: 'conn-1', source: 'action-1', target: 'action-2' }
    ]
  }
  ...
  if (result.hasConflicts) {
    ...
    // Try auto-resolve
    const autoResult = await conflictResolutionService.autoResolve(result.conflicts)
...
export function operationalTransformExample() {
  ...
  console.log('Equal?', JSON.stringify(doc1Final) === JSON.stringify(doc2Final))
}
...
export async function complexWorkflowMerge() {
  const baseWorkflow = {
    ...
    actions: [
      ...
      { id: 'a2', type: 'type', selector: '#username', text: 'user', x: 200, y: 100 },
      { id: 'a3', type: 'type', selector: '#password', text: 'pass', x: 300, y: 100 }
    ],
    connections: [
  ...
  // Merge
  const detector = conflictResolutionService.getDetector()
  const mergeResult = detector.threeWayMerge(userAVersion, userBVersion, baseWorkflow)
...
export function concurrentActionModifications() {
  ...
  const detector = conflictResolutionService.getDetector()
  const conflicts = detector.detectConflicts(user1Action, user2Action, baseAction)
