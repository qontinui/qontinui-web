...
export const PROMPT_TEMPLATES: Record<string, PromptTemplate> = {
  ...
  web_scraping: {
    ...
    template: 'Create a workflow that scrapes {target} and extracts {data_points}',
    examples: [
  ...
  data_processing: {
    ...
    parameters: [
      { name: 'data_source', placeholder: 'where data comes from', required: true },
  ...
  monitoring: {
    ...
    parameters: [
      ...
      { name: 'change_type', placeholder: 'what changes to look for', required: true },
  ...
  batch_processing: {
    ...
    parameters: [
      ...
      { name: 'operation', placeholder: 'what to do with each', required: true },
    ],
  ...
  error_handling: {
    ...
    parameters: [
      ...
      { name: 'fallback_action', placeholder: 'what to do on error', required: true },
    ],
 ...
 */
export function fillTemplate(template: PromptTemplate, params: Record<string, string>): string {
...
export function getExamplePrompts(template: PromptTemplate): string[] {
  return template.examples.map(example => {
    const params = template.parameters?.reduce((acc, param) => {
