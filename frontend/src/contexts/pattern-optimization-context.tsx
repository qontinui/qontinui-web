...
interface PatternOptimizationContextType {
  ...
  labelScreenshot: (id: string, label: 'positive' | 'negative' | 'unlabeled') => void
  ...
  // Strategy evaluation
  evaluateStrategy: (strategy: OptimizationStrategy) => Promise<StrategyEvaluation>
  evaluations: StrategyEvaluation[]
  selectedStrategy: OptimizationStrategy | null
  selectStrategy: (strategy: OptimizationStrategy) => void

  // Results
  generateResult: (selectedPatternIds?: Set<string>) => Promise<OptimizationResult | null>
...
export function PatternOptimizationProvider({ children }: PatternOptimizationProviderProps) {
  ...
  useEffect(() => {
    const storedSession = localStorage.getItem('pattern-optimization-session-meta')
    if (storedSession) {
      try {
  ...
  useEffect(() => {
    if (session) {
      ...
      try {
        localStorage.setItem('pattern-optimization-session-meta', JSON.stringify(lightSession))
      } catch (error) {
  ...
  const addScreenshots = useCallback(async (screenshots: OptimizationScreenshot[]) => {
    ...
    const processedScreenshots = await Promise.all(
      screenshots.map(async (screenshot) => {
        try {
          await patternOptimizationStorage.storeImage(screenshot.id, screenshot.url)
  ...
  const labelScreenshot = useCallback((id: string, label: 'positive' | 'negative' | 'unlabeled') => {
    setSession(prev => {
      if (!prev) return prev
      return {
        ...prev,
  ...
  const copyRegionToAll = useCallback((sourceId: string) => {
    setSession(prev => {
      if (!prev) return prev
      const sourceScreenshot = prev.screenshots.find(s => s.id === sourceId)
  ...
  const startAnalysis = useCallback(async () => {
    ...
    try {
      ...
      if (positiveScreenshots.length === 0 || regions.length === 0) {
        console.error('No positive screenshots with regions', { positiveScreenshots, regions })
      ...
      const response = await fetch('http://localhost:8001/api/v1/optimize-pattern', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          regions: regions,
          strategies: ['multi-pattern', 'consensus', 'feature-based', 'differential']
        })
      })
      ...
      const patterns: ExtractedPattern[] = apiResult.extractedPatterns.map((pattern: any, idx: number) => ({
        id: pattern.id,
        screenshotId: session.screenshots[pattern.screenshot_index]?.id || `screenshot_${pattern.screenshot_index}`,
      ...
      const analysis: PatternAnalysis = {
        ...
        similarityMatrix: { patterns, scores: apiResult.similarityMatrix.scores },
  ...
  const evaluateStrategy = useCallback(async (strategy: OptimizationStrategy): Promise<StrategyEvaluation> => {
    if (!session?.analysis) {
      throw new Error("No analysis available")
    }

    // Use the evaluations from the API response if available
    const existingEvaluation = session.evaluations.find(e => e.strategy.type === strategy.type)
    ...
    setSession(prev => {
      if (!prev) return prev
      const evaluations = [...prev.evaluations.filter(e => e.strategy.type !== strategy.type), evaluation]
      return {
        ...prev,
        evaluations,
        updatedAt: new Date(),
  ...
  const selectStrategy = useCallback((strategy: OptimizationStrategy) => {
    setSession(prev => prev ? {
      ...prev,
  ...
  const updatePattern = useCallback((patternId: string, customMask: string) => {
    setSession(prev => {
      if (!prev || !prev.analysis) return prev

      const updatedPatterns = prev.analysis.extractedPatterns.map(pattern =>
  ...
  const generateResult = useCallback(async (selectedPatternIds?: Set<string>): Promise<OptimizationResult | null> => {
    ...
    try {
      ...
      if (selectedPatternIds && selectedPatternIds.size > 0) {
        patternsToUse = session.analysis.extractedPatterns.filter(p => selectedPatternIds.has(p.id))
      }
      ...
      const response = await fetch('http://localhost:8001/api/v1/create-state-image', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          ...
          similarity_threshold: session.selectedStrategy.parameters?.threshold || 0.8
        })
      })
