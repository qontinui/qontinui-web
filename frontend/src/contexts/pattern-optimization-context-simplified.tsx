...
const PatternOptimizationContext = createContext<PatternOptimizationContextType | undefined>(undefined)

export const usePatternOptimization = () => {
  const context = useContext(PatternOptimizationContext)
  if (!context) {
    throw new Error("usePatternOptimization must be used within a PatternOptimizationProvider")
  }
  return context
}
 ...
 */
export function PatternOptimizationProvider({ children }: PatternOptimizationProviderProps) {
  ...
  useEffect(() => {
    if (session) {
      try {
        localStorage.setItem('pattern-optimization-session', JSON.stringify(session))
      } catch (error) {
  ...
  const addScreenshots = useCallback(async (files: File[]) => {
    ...
    console.log('[PatternOptimization] Added screenshots:', newScreenshots.length)
  }, [session])
  ...
  const setAllScreenshotRegions = useCallback((region: Region) => {
    ...
    console.log('[PatternOptimization] Set region for all screenshots:', region)
  }, [])
  ...
  const extractPattern = useCallback(async (config: ExtractionConfig) => {
    ...
    const screenshotsWithRegions = session.screenshots.filter(s => s.region)
    if (screenshotsWithRegions.length === 0) {
      throw new Error("No regions selected for pattern extraction")
    }

    console.log('[PatternOptimization] Extracting pattern with config:', config)
    ...
    try {
      ...
      const extractRegion = async (dataUrl: string, region: Region): Promise<string> => {
      ...
      for (const screenshot of screenshotsWithRegions) {
        ...
        const imageData = await patternOptimizationStorage.getImage(screenshot.id)
        if (imageData && screenshot.region) {
          // Extract only the region, not the full image
          const croppedImage = await extractRegion(imageData, screenshot.region)
      ...
      console.log(`[PatternOptimization] Sending ${screenshots.length} cropped regions`)

      // Call API for pattern extraction
      console.log('[PatternOptimization] Sending API request to extract pattern...')
      const requestBody = {
      ...
      console.log('[PatternOptimization] Request size:', (requestSize / 1024 / 1024).toFixed(2), 'MB');
      console.log('[PatternOptimization] Request body summary:', {
        screenshots: requestBody.screenshots.length,
        screenshotSizes: requestBody.screenshots.map(s => (s.length / 1024).toFixed(1) + 'KB'),
      ...
      const timeoutId = setTimeout(() => {
        console.error('[PatternOptimization] Aborting request due to timeout');
        controller.abort();
      }, 60000); // 60 second timeout

      let response;
      try {
        console.log('[PatternOptimization] Sending fetch request to:', 'http://localhost:8000/api/masked-patterns/extract-masked');
      ...
      console.log('[PatternOptimization] API response status:', response.status, response.statusText)
      console.log('[PatternOptimization] Response headers:', response.headers)
      ...
      try {
        ...
        console.log('[PatternOptimization] Raw response text length:', responseText.length);
        result = JSON.parse(responseText);
        console.log('[PatternOptimization] Pattern extracted successfully:', result);
      } catch (parseError) {
        console.error('[PatternOptimization] Failed to parse response:', parseError);
        throw new Error('Failed to parse API response');
      }

      // Get detailed pattern data including images
      const detailsResponse = await fetch(`/api/masked-patterns/${result.id}`)
      if (!detailsResponse.ok) {
        throw new Error(`Failed to get pattern details: ${detailsResponse.statusText}`)
      }
      ...
      console.log('[PatternOptimization] Extraction successful! Pattern saved to session:', extractedPattern)

    } catch (error: any) {
      console.error('[PatternOptimization] Extraction failed:', error)

      if (error.name === 'AbortError') {
        console.error('[PatternOptimization] Request timed out after 60 seconds')
      }
      ...
      console.log('[PatternOptimization] Extraction finished, isExtracting set to false')
    }
  }, [session])

  const analyzePatternQuality = useCallback((pattern: ExtractedPattern): PatternQuality => {
    ...
    if (density < 0.1) {
      recommendations.push('Pattern has very low mask density - consider lowering similarity threshold')
    } else if (density > 0.9) {
      recommendations.push('Pattern has very high mask density - may be too general')
    }
    ...
    if (avgConf < 0.7) {
      recommendations.push('Low average confidence - screenshots may be too different')
    }
  ...
  const exportPattern = useCallback((pattern: ExtractedPattern) => {
    const exportData = {
      pattern: {
        id: pattern.id,
        name: pattern.name,
