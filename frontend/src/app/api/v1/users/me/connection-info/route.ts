...
export async function GET(request: NextRequest) {
  try {
    ...
    if (accessTokenCookie?.value) {
      ...
      console.log('[Connection-Info API Route] Using token from Authorization header')
    }

    if (!accessToken) {
      console.error('[Connection-Info API Route] No access token found in cookie or header')
    ...
    console.log('[Connection-Info API Route] Attempting to fetch from backend...')
    ...
    console.log('[Connection-Info API Route] Backend response status:', response.status)
    console.log('[Connection-Info API Route] Backend response ok:', response.ok)
    ...
    console.error('[Connection-Info API Route] Error name:', (error as Error).name)
    console.error('[Connection-Info API Route] Error message:', (error as Error).message)
