...
function VerifyStatesContent() {
  ...
  const [highlightElementId, setHighlightElementId] = useState<string | undefined>()
  ...
  const transitionCounts = useMemo(() => {
    ...
    const outgoing = transitions.filter(t =>
      t.type === 'OutgoingTransition' && t.fromState === selectedState.id
    ).length

    const incoming = transitions.filter(t =>
    ...
    <div className="container mx-auto py-8 h-[calc(100vh-4rem)] flex flex-col">
      ...
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">
          Visualize how states look with their elements positioned at configured fixed positions
        </p>
      </div>
