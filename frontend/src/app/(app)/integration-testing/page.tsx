...
export default function IntegrationTestingPage() {
  ...
  const handleExecute = async (processId: string, initialStates: string[], snapshotRunIds: string[]) => {
    ...
    const actions: ActionSpec[] = process.actions?.map((action: any) => ({
    ...
    <div className="p-6 space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
              ...
              {result ? (
                ...
                <div className="flex items-center justify-center h-96 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
                  <div className="text-center text-gray-500">
                    ...
                    <p className="text-lg font-medium">Executing process...</p>
                    <p className="text-sm mt-1">Please wait while the process runs</p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-center h-96 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
                  <div className="text-center text-gray-500">
                    ...
                    <p className="text-sm mt-1">
                      Select a process and configure initial states, then click Execute
                    </p>
                  </div>
                </div>
              ) : (
                <div className="flex items-center justify-center h-96 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
                  <div className="text-center text-gray-500">
                    <p className="text-lg font-medium">No snapshots selected</p>
                    <p className="text-sm mt-1">
        ...
        <TabsContent value="coverage">
          <StateCoveragePanel
            processId={selectedProcessId}
            processName={workflows.find((w) => w.id === selectedProcessId)?.name}
        ...
        <TabsContent value="snapshots">
          <SnapshotListCard
            onSelect={(snapshot) => {
              ...
              if (isSelected) {
                setSelectedSnapshots(selectedSnapshots.filter((s) => s.id !== snapshot.id));
              } else {
