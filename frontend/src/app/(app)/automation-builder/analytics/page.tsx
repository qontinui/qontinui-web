...
type TimeRangePreset = 'today' | 'week' | 'month' | 'quarter' | 'year' | 'custom';
...
function getComplexityLevel(actionCount: number): 'low' | 'medium' | 'high' | 'very-high' {
...
function MetricCard({ title, value, icon, trend, description, variant = 'default', color }: MetricCardProps) {
  const variantStyles = {
...
function ExecutionTable({ executions, onRowClick }: ExecutionTableProps) {
  const [sortBy, setSortBy] = useState<'startTime' | 'duration' | 'workflowName'>('startTime');
              ...
              {paginatedExecutions.map((execution) => (
                ...
                >
                  <td className="p-4 align-middle">
                    ...
                    <div className="text-xs text-muted-foreground">{execution.workflowId.substring(0, 8)}...</div>
                  </td>
                  <td className="p-4 align-middle">
                    <div className="text-sm">{new Date(execution.startTime).toLocaleString()}</div>
                    <div className="text-xs text-muted-foreground">{formatRelativeTime(execution.startTime)}</div>
                  </td>
                  <td className="p-4 align-middle">
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between">
          <p className="text-sm text-muted-foreground">
            Showing {(currentPage - 1) * itemsPerPage + 1} to {Math.min(currentPage * itemsPerPage, executions.length)}{' '}
...
export default function WorkflowAnalyticsPage() {
  ...
  // State
  const [timeRange, setTimeRange] = useState<{ start: Date; end: Date }>(TIME_RANGES.week());
  const [timeRangePreset, setTimeRangePreset] = useState<TimeRangePreset>('week');
  ...
  const filteredMetrics = useMemo(() => {
    ...
    if (filters.searchQuery) {
      ...
      filtered = filtered.filter(m => m.workflowName.toLowerCase().includes(query));
    }
  ...
  const topWorkflows = useMemo(() => {
    ...
    const highestError = workflowAnalyticsService.getHighestErrorRateWorkflows(10);
    const recentlyFailed = recentExecutions.filter(e => !e.success).slice(0, 10);
  ...
  const handleExportCSV = () => {
    const headers = ['Workflow', 'Total Executions', 'Success Rate', 'Avg Duration', 'Last Executed'];
      ...
      <div className="max-w-[1800px] mx-auto space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-400">Monitor execution, performance, and usage metrics</p>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
          ...
          <MetricCard
            ...
            variant={aggregatedStats.overallSuccessRate > 0.9 ? 'success' : aggregatedStats.overallSuccessRate > 0.7 ? 'warning' : 'error'}
            color="#00FF88"
          />
          <MetricCard
          ...
          <MetricCard
            title="Most Active"
            value={aggregatedStats.mostActiveWorkflow?.workflowName.substring(0, 15) || 'N/A'}
            icon={<TrendingUp className="h-4 w-4" />}
            description={aggregatedStats.mostActiveWorkflow ? `${aggregatedStats.mostActiveWorkflow.executionCount} runs` : 'No data'}
            color="#FFD700"
          />
          <MetricCard
            title="Slowest"
            value={aggregatedStats.slowestWorkflow?.workflowName.substring(0, 15) || 'N/A'}
            icon={<AlertTriangle className="h-4 w-4" />}
            description={aggregatedStats.slowestWorkflow ? formatDuration(aggregatedStats.slowestWorkflow.avgDuration) : 'No data'}
            color="#FF6B6B"
          />
        </div>
        ...
        <Tabs defaultValue="dashboard" className="space-y-6">
          ...
          <TabsContent value="dashboard" className="space-y-6">
            <AnalyticsDashboard
              workflows={[]}
              metrics={filteredMetrics.reduce((acc, m) => ({ ...acc, [m.workflowId]: m }), {})}
              timeRange={timeRange}
                    ...
                    <div className="space-y-3">
                      {topWorkflows.mostExecuted.map((metric, index) => (
                        <div
                          ...
                          onClick={() => setSelectedWorkflow(metric.workflowId)}
                        >
                          ...
                          <div className="text-right">
                            <p className="text-2xl font-bold text-[#00D9FF]">{formatNumber(metric.totalExecutions)}</p>
              ...
              <TabsContent value="slowest">
                <Card className="bg-[#1A1A1B]/50 border-gray-800/50">
                  <CardHeader>
                    ...
                    <CardDescription>Top 10 workflows by average duration</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-3">
                      {topWorkflows.slowest.map((metric, index) => (
                        ...
                        >
                          ...
                          <div className="text-right">
                            <p className="text-2xl font-bold text-[#BD00FF]">{formatDuration(metric.avgDuration)}</p>
                  ...
                  <CardContent>
                    {topWorkflows.highestError.length > 0 ? (
                      ...
                      <div className="text-center py-12">
                        ...
                        <h3 className="text-lg font-semibold mb-2">No Failures Detected</h3>
                        <p className="text-muted-foreground">All workflows are running successfully!</p>
                      </div>
                    )}
                  </CardContent>
                </Card>
                  ...
                  <CardContent>
                    {topWorkflows.recentlyFailed.length > 0 ? (
                      ...
                      <div className="text-center py-12">
                        ...
                        <p className="text-muted-foreground">All recent executions were successful!</p>
                      </div>
                    )}
                  </CardContent>
                </Card>
          ...
          <TabsContent value="executions">
            <Card className="bg-[#1A1A1B]/50 border-gray-800/50">
              <CardHeader>
                ...
                <CardDescription>Recent workflow executions with detailed information</CardDescription>
              </CardHeader>
              <CardContent>
                <ExecutionTable
            ...
            {selectedWorkflow && selectedWorkflowData ? (
              <>
                <WorkflowMetricsPanel
                  ...
                  complexityMetrics={selectedWorkflowData.complexityMetrics as any}
                  executionHistory={selectedWorkflowData.executionHistory as any}
                />
                <PerformanceAnalyzer
                  workflow={selectedWorkflowData.workflow as any}
                  onAnalyze={() => {}}
              ...
              <Card className="bg-[#1A1A1B]/50 border-gray-800/50">
                <CardContent className="flex flex-col items-center justify-center py-16">
                  ...
                  <p className="text-muted-foreground text-center mb-6">
                    Choose a workflow from the Top Workflows tab to view detailed performance analysis
                  </p>
                  <Button
                    onClick={() => {
            ...
            <TabsContent value="comparison">
              <Card className="bg-[#1A1A1B]/50 border-gray-800/50">
                <CardHeader>
                  ...
                  <CardDescription>
                    Comparing {comparisonWorkflows.length} workflow{comparisonWorkflows.length !== 1 ? 's' : ''}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <Alert className="mb-6">
                    ...
                    <AlertDescription>
                      This feature is coming soon! You'll be able to compare execution counts, durations, success
                      rates, and complexity side-by-side.
                    </AlertDescription>
                  </Alert>
                </CardContent>
              </Card>
          ...
          <Alert className="fixed bottom-4 right-4 w-96 bg-[#1A1A1B] border-gray-700">
            ...
            <AlertDescription>
              <div className="space-y-2 mt-2">
                ...
                <p className="text-xs text-muted-foreground">{new Date(selectedExecution.startTime).toLocaleString()}</p>
                <div className="flex items-center gap-2">
                  <Badge variant={selectedExecution.success ? 'default' : 'destructive'}>
                    {selectedExecution.success ? 'Success' : 'Failed'}
                  </Badge>
                  <Badge variant="outline">{formatDuration(selectedExecution.duration)}</Badge>
                </div>
                {selectedExecution.error && (
                  <div className="mt-2 p-2 rounded bg-red-500/10 text-xs">{selectedExecution.error}</div>
                )}
              </div>
            </AlertDescription>
          </Alert>
