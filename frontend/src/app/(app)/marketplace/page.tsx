...
export default function MarketplacePage() {
  ...
  const [selectedTab, setSelectedTab] = useState<'all' | 'popular' | 'installed'>('all')
  const [installDialogOpen, setInstallDialogOpen] = useState(false)
  const [selectedPackage, setSelectedPackage] = useState<CodePackage | null>(null)
  const [installStatus, setInstallStatus] = useState<InstallStatus>('idle')

  // Queries
  const { data: searchResult, isLoading: isLoadingSearch } = usePackages(searchFilters)
  const { data: popularPackages, isLoading: isLoadingPopular } = usePopularPackages()
  ...
  const isPackageInstalled = (packageId: string) => {
    return installedPackages?.some((install) => install.package_id === packageId) || false
  }

  const displayedPackages = selectedTab === 'popular'
    ? popularPackages || []
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="border-b border-gray-800/50 bg-gradient-to-b from-cyan-500/5 via-purple-500/5 to-transparent">
        <div className="container mx-auto px-6 py-8">
          <div className="flex items-start justify-between mb-6">
            <div>
              ...
              <p className="text-gray-400 text-lg">
                Discover and share automation packages created by the community
              </p>
            </div>
            <Button
          ...
          {/* Search Bar */}
          <PackageSearchBar onSearch={handleSearch} initialFilters={searchFilters} />
        </div>
      </div>
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        {/* Pagination */}
        {selectedTab === 'all' && searchResult && searchResult.total_pages > 1 && (
  ...
  if (packages.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <h3 className="text-xl font-semibold text-gray-400 mb-2">No packages found</h3>
        <p className="text-gray-500">
          Try adjusting your search filters or check back later for new packages.
        </p>
      </div>
    )
  }
