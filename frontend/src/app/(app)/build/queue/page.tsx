...
export default function QueuePage() {
  const { data: queueItems, isLoading, error, isOffline, refetch } =
    useWorkflowQueue();
  const {
    data: workflows,
  ...
  const filteredQueue = useMemo(() => {
    ...
    return queueItems.filter((item) => {
      const name = (item.name as string) || (item.workflow_name as string) || "";
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-surface-canvas via-[#0F0F10] to-surface-canvas text-white">
      <header className="border-b border-border-subtle/50 bg-surface-canvas/80 backdrop-blur-xl sticky top-0 z-50">
        <div className="flex items-center justify-between px-6 py-4">
          <div className="flex items-center gap-4">
            <ListOrdered className="w-6 h-6 text-brand-primary" />
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              className={showLibrary ? "bg-brand-primary text-black" : "border-border-default"}
            >
            ...
            {queueItems && queueItems.length > 0 && (
              <Button
                ...
                onClick={() => toast.info("Clear queue requested - this feature requires runner support")}
              >
      ...
      <main className="p-6 mx-auto flex gap-6" style={{ maxWidth: showLibrary ? "1400px" : "1024px" }}>
        {showLibrary && (
            ...
            <Card className="bg-surface-raised/50 border-border-subtle/50">
              ...
              <CardContent className="space-y-3">
                ...
                <div className="space-y-2 max-h-[500px] overflow-y-auto">
                  {(workflows || [])
                    .filter(w => !librarySearch || w.name.toLowerCase().includes(librarySearch.toLowerCase()))
                    .map((workflow) => (
                      <div key={workflow.id} className="p-3 rounded-lg bg-surface-canvas/50 border border-border-subtle/30 hover:border-border-default transition-all">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-sm font-medium text-text-primary truncate">{workflow.name}</span>
                        </div>
                        {workflow.description && (
                          <p className="text-xs text-text-muted mb-2 line-clamp-2">{workflow.description}</p>
                        )}
                        <Button
                          variant="outline"
                          size="sm"
                          ...
                          onClick={async () => {
                            try {
                              ...
                              refetch();
                              toast.success(`Added "${workflow.name}" to queue`);
                            } catch {
                  ...
                  {workflows && workflows.length === 0 && (
                    <p className="text-xs text-text-muted text-center py-4">No workflows available</p>
                  )}
                </div>
              </CardContent>
            </Card>
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="w-8 h-8 bg-surface-hover rounded-lg flex items-center justify-center shrink-0">
                        <span data-content-role="metric" data-content-label="queue position" className="text-sm font-mono text-text-muted">
                          {index + 1}
                        </span>
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 text-xs text-text-muted">
                          {item.created_at ? (
                            <span data-content-role="metric" data-content-label="created timestamp" className="flex items-center gap-1">
        ...
        {filteredQueue && filteredQueue.length > 0 && (
          <div className="mt-4 flex items-center justify-between">
            <div className="flex items-center gap-3 text-sm text-text-muted">
              <span data-content-role="metric" data-content-label="queue item count">{filteredQueue.length} item{filteredQueue.length !== 1 ? "s" : ""} in queue</span>
              <span className="text-xs">&bull;</span>
              <span data-content-role="metric" data-content-label="total steps count" className="text-xs">{filteredQueue.reduce((sum, item) => {
