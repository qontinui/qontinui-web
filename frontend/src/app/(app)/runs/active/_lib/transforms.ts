...
export function transformScreenshots(raw: unknown): Screenshot[] {
...
export function transformVerification(raw: unknown): VerificationData {
  ...
  if (obj && typeof obj === "object" && "results" in obj && Array.isArray(obj.results)) {
    return {
      ...
      summary: (obj.summary as import("@/lib/runner").VerificationSummary) ?? null,
    };
  }
  if (Array.isArray(raw))
    return { results: raw as import("@/lib/runner").VerificationResult[], summary: null };
...
export function transformMcpCalls(raw: unknown): McpCall[] {
  const obj = raw as Record<string, unknown>;
