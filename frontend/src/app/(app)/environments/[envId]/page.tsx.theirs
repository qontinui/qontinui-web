...
import { MachineConfigHistory } from "../_components/MachineConfigHistory";
import {
  DevenvApiError,
  DRIFT_POLL_MS,
  getEnvironment,
...
export default function EnvironmentDetailPage() {
  ...
  /** Machine whose config-history panel is expanded (one at a time). */
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="rounded-lg border border-border">
        <div className="px-4 py-3 border-b border-border bg-muted/50">
          ...
          <p className="text-xs text-muted-foreground">
            Each machine&apos;s capture timeline — expand a machine to browse
            change points and diff two captures
          </p>
        </div>
        <div className="p-4">
          {eligibleMachines.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-4">
              No machines have reported a config for this environment yet.
            </p>
          ) : (
            <div className="space-y-2">
              {eligibleMachines.map((machine) => {
                const open = historyMachineId === machine.id;
                return (
                  <div
                    key={machine.id}
                  ...
                  >
                    <button
                      type="button"
                      onClick={() =>
                        setHistoryMachineId(open ? null : machine.id)
                      }
                    ...
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        {environment.canonical_machine_id === machine.id && (
