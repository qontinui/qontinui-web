...
const SCREENSHOT_PROXY_BASE = "/api/v1/screenshots";
const TEXT_PREVIEW_LIMIT = 500;

/** Build the screenshot proxy URL from a relative screenshot path. */
function screenshotUrl(path: string): string {
  // Strip leading '.dev-logs/' or '.dev-logs\' prefix so the proxy can resolve it
  const clean = path
    .replace(/^\.?[/\\]?dev-logs[/\\]/, "")
    .replace(/^\.dev-logs[/\\]/, "");
...
/** Collapsible text block with truncation and expand/collapse. */
...
/** Screenshot evidence with lazy loading and full-size link. */
...
/** Evidence section that aggregates all evidence types for a step.
 *
 * Shows screenshot, DOM snapshot, and log output evidence.
 * Note: console_output from verification_details is rendered in the main
 * step card above, so it's not duplicated here.
 */
function EvidenceSection({ step }: { step: VerificationStepResult }) {
    ...
    <div className="border border-border-subtle/30 rounded-lg overflow-hidden">
      ...
      {expanded && (
        <div className="border-t border-border-subtle/30 px-4 py-3 bg-surface-canvas/30 space-y-3">
          ...
          {/* Evidence: screenshots, DOM snapshots, log output */}
