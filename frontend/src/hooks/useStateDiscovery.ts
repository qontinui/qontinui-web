...
export function useStateDiscovery() {
  ...
  const uploadScreenshots = useCallback(async (files: File[]) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}${API_PATH}/state-discovery/upload`,
        {
          method: "POST",
          body: formData,
        }
        ...
        switch (message.type) {
          ...
          case "error":
            ...
            const errorMessage =
              ...
              message.data?.error ||
              "An error occurred during state discovery";
    ...
    async (
      ...
      setStates((prev) => {
        const newStates = prev
          .map((state) => ({
            ...state,
            stateImageIds: state.stateImageIds.filter(
              (id) => id !== stateImageId
            ),
          }))
  ...
  const updateStateImage = useCallback(
    async (stateImageId: string, updates: Partial<StateImage>) => {
      ...
      // Update local state
      setStateImages((prev) =>
        prev.map((si) => (si.id === stateImageId ? { ...si, ...updates } : si))
      );
