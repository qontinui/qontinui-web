...
export function useSnapshotRecommendations(
  params?: UseSnapshotRecommendationsParams
) {
  ...
  const [selectedRecommendation, setSelectedRecommendation] =
    useState<SnapshotRecommendation | null>(null);
  ...
  const fetchRecommendations = async () => {
    ...
    try {
      ...
      if (params?.max_snapshots)
        searchParams.set("max_snapshots", params.max_snapshots.toString());
      if (params?.num_recommendations)
        searchParams.set(
          "num_recommendations",
          params.num_recommendations.toString()
        );

      const response = await fetch(
        `/api/snapshots/recommendations?${searchParams}`
      );
