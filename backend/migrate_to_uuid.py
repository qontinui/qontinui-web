...
with engine.begin() as conn:
    ...
    conn.execute(text('ALTER TABLE users ADD COLUMN IF NOT EXISTS new_id UUID DEFAULT gen_random_uuid() NOT NULL'))
    conn.execute(text('ALTER TABLE users ADD CONSTRAINT users_new_id_unique UNIQUE (new_id)'))

    # Save mapping
    conn.execute(text('INSERT INTO _id_mapping (table_name, old_id, new_id) SELECT \'users\', id, new_id FROM users ON CONFLICT DO NOTHING'))

    print("\n4. Adding UUID columns to projects table...")
    conn.execute(text('ALTER TABLE projects ADD COLUMN IF NOT EXISTS new_id UUID DEFAULT gen_random_uuid() NOT NULL'))
    conn.execute(text('ALTER TABLE projects ADD COLUMN IF NOT EXISTS new_owner_id UUID'))
    conn.execute(text('ALTER TABLE projects ADD CONSTRAINT projects_new_id_unique UNIQUE (new_id)'))
    ...
    # Save mapping
    conn.execute(text('INSERT INTO _id_mapping (table_name, old_id, new_id) SELECT \'projects\', id, new_id FROM projects ON CONFLICT DO NOTHING'))
    ...
    for table_name, column_name in result:
        ...
        conn.execute(text(f'ALTER TABLE {table_name} ADD COLUMN IF NOT EXISTS new_{column_name} UUID'))
        conn.execute(text(f'''
    ...
    for table_name, column_name in result:
        ...
        conn.execute(text(f'ALTER TABLE {table_name} DROP COLUMN IF EXISTS {old_column} CASCADE'))
        conn.execute(text(f'ALTER TABLE {table_name} RENAME COLUMN {column_name} TO {old_column}'))
        conn.execute(text(f'''
    ...
    '''))
    conn.execute(text('CREATE INDEX ix_session_activities_id ON session_activities(id)'))
    conn.execute(text('CREATE INDEX ix_session_activities_user_id ON session_activities(user_id)'))
    conn.execute(text('CREATE INDEX ix_session_activities_jti ON session_activities(jti)'))
    ...
    conn.execute(text('CREATE INDEX ix_device_sessions_user_id ON device_sessions(user_id)'))
    conn.execute(text('CREATE INDEX ix_device_sessions_device_fingerprint ON device_sessions(device_fingerprint)'))
    ...
    '''))
    conn.execute(text('CREATE INDEX ix_analytics_events_event_name ON analytics_events(event_name)'))
    conn.execute(text('CREATE INDEX ix_analytics_events_user_id ON analytics_events(user_id)'))
    conn.execute(text('CREATE INDEX ix_analytics_events_timestamp ON analytics_events(timestamp)'))
    conn.execute(text('CREATE INDEX ix_analytics_events_name_timestamp ON analytics_events(event_name, timestamp)'))
    conn.execute(text('CREATE INDEX ix_analytics_events_user_name ON analytics_events(user_id, event_name)'))
...
with engine.connect() as conn:
    # Check users table
    result = conn.execute(text("SELECT data_type FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'id'"))
    user_id_type = result.scalar()
    print(f"✓ users.id type: {user_id_type}")

    # Check projects table
    result = conn.execute(text("SELECT data_type FROM information_schema.columns WHERE table_name = 'projects' AND column_name = 'id'"))
