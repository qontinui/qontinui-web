...
``PUT  /overview/settings``                  save them (tenant admin)
``GET  /overview/estimates``                 list this project's estimates
``POST /overview/estimates``                 create one (tenant admin)
``GET  /overview/estimates/{id}``            head row + whole content graph
``PATCH /overview/estimates/{id}``           edit the head row (tenant admin)
``DELETE /overview/estimates/{id}``          delete it and its content (tenant admin)
``PUT  /overview/estimates/{id}/content``    replace the content graph (tenant admin)
...
2. **Reads are open to any member; every write is tenant-admin.** The plan's
   design decision 3 — a business leader is invited as a coord ``operator``,
   whose overview access is read-only because there is no write that is not
   admin-gated. Reads deliberately do NOT require admin.
...
from app.models.overview import Estimate, EstimateRole, Phase
...
async def get_overview_tenant_id(
    ...
    effective tenant instead, the same way
    ``require_coord_tenant_admin_target`` does for writes, so a read and a
    write in the same session can never land in different projects.

    The selection is membership-validated inside ``_effective_tenant_id``
    (mirroring coord's own ``apply_active_tenant_override``): a tenant the
    operator does not belong to degrades to home rather than widening access.
    """
...
def _actor(user: UserModel | None) -> str | None:
    if user is None:
        return None
...
async def _require_estimate(
    ...
    row = await crud.get_estimate(db, tenant_id=tenant_id, estimate_id=estimate_id)
    if row is None:
...
async def write_settings(
    ...
    """Save the project's settings. Tenant admin only."""
    try:
...
async def create_estimate(
    ...
    """Create an estimate. Tenant admin only.
...
async def patch_estimate(
    ...
    """Edit the head row. Tenant admin only.

    Absent fields are left alone; an explicit ``null`` clears a nullable one.
    """
    row = await _require_estimate(db, tenant_id, estimate_id)
...
async def delete_estimate(
    ...
    """Delete an estimate and everything under it. Tenant admin only."""
    row = await _require_estimate(db, tenant_id, estimate_id)
    await crud.delete_estimate(db, estimate=row)
    await db.commit()
...
async def replace_estimate_content(
    ...
    """Replace the estimate's whole content graph. Tenant admin only.
    ...
    try:
        await crud.replace_content(
            db, estimate=row, actor=_actor(current_user), content=payload
        )
    except crud.VersionConflict as exc:
        raise _version_conflict(exc) from exc
    await db.commit()
