...
class DiscoveredState(Base):
    ...
    screenshot_ids: Mapped[list[UUID] | None] = mapped_column(
        ARRAY(PGUUID(as_uuid=True)), nullable=True, server_default="{}"
    )

    # Frame IDs belonging to this state (from recordings)
    frame_ids: Mapped[list | None] = mapped_column(
    ...
    strings: Mapped[list | None] = mapped_column(
        JSON, nullable=True, server_default="[]"
    )
    ...
    is_initial: Mapped[bool | None] = mapped_column(
        Boolean, nullable=True, default=False
    )
    is_error_state: Mapped[bool | None] = mapped_column(
        Boolean, nullable=True, default=False
    )
    is_transient: Mapped[bool | None] = mapped_column(
        Boolean, nullable=True, default=False
    )
    ...
    user_edited: Mapped[bool | None] = mapped_column(
        Boolean, nullable=True, default=False
    )
    user_approved: Mapped[bool | None] = mapped_column(
        Boolean, nullable=True, default=False
    )
    user_notes: Mapped[str | None] = mapped_column(Text, nullable=True)

    # Conversion to actual state (from recordings)
    converted_to_state_id: Mapped[UUID | None] = mapped_column(
    ...
    recording: Mapped["Recording | None"] = relationship(
        "Recording", back_populates="discovered_states", foreign_keys=[recording_id]
    )
    ...
    def __repr__(self) -> str:
        source_id = (
            self.automation_session_id
            if self.source_type == "automation_session"
