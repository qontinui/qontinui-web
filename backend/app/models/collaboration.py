...
class ProjectComment(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, server_default="gen_random_uuid()")
    project_id = Column(Integer, ForeignKey("projects.id", ondelete="CASCADE"), nullable=False, index=True)
    workflow_id = Column(String, nullable=True, index=True)  # Optional: specific workflow
    action_id = Column(String, nullable=True, index=True)  # Optional: specific action/state
    author_id = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    content = Column(Text, nullable=False)
    position = Column(JSON, nullable=True)  # {x: number, y: number} for canvas positioning
    mentions = Column(JSON, nullable=True)  # Array of user IDs mentioned in comment
    resolved = Column(Boolean, default=False, nullable=False)
    resolved_by = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    resolved_at = Column(DateTime, nullable=True)
    parent_comment_id = Column(
        UUID(as_uuid=True), ForeignKey("project_comments.id", ondelete="CASCADE"), nullable=True, index=True
    )
    ...
    resolver = relationship("User", foreign_keys=[resolved_by], backref="resolved_comments")
