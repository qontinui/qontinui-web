...
class JSONConfigValidator:
    ...
    SUPPORTED_ACTION_TYPES = [
        "FIND", "CLICK", "DOUBLE_CLICK", "RIGHT_CLICK", 
        "TYPE", "KEY_PRESS", "DRAG", "SCROLL", "WAIT",
        "VANISH", "EXISTS", "MOVE", "SCREENSHOT", 
        "CONDITION", "LOOP"
    ]
        ...
        try:
            ...
            for process in config.processes:
                ...
                for action in process.actions:
                    ...
                    if action.config.target and action.config.target.get("type") == "image":
                        image_id = action.config.target.get("imageId")
                        if image_id and image_id not in image_ids:
                            errors.append(f"Action references non-existent image: {image_id}")
            ...
            for state in config.states:
                ...
                for state_image in state.identifyingImages:
                    if state_image.imageId not in image_ids:
                        errors.append(f"State references non-existent image: {state_image.imageId}")
            ...
            for transition in config.transitions:
                ...
                for process_id in transition.processes:
                    if process_id not in process_ids:
                        errors.append(f"Transition references non-existent process: {process_id}")
                
                # Validate state references
                if transition.fromState and transition.fromState not in state_ids:
                    errors.append(f"Transition references non-existent fromState: {transition.fromState}")
                
                if transition.toState and transition.toState not in state_ids:
                    errors.append(f"Transition references non-existent toState: {transition.toState}")
                
                for state_id in transition.activateStates:
                    if state_id not in state_ids:
                        errors.append(f"Transition references non-existent state to activate: {state_id}")
                
                for state_id in transition.deactivateStates:
                    if state_id not in state_ids:
                        errors.append(f"Transition references non-existent state to deactivate: {state_id}")
            ...
            if orphaned_processes:
                warnings.append(f"Orphaned processes not used in any transition: {orphaned_processes}")
