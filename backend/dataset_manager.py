...
class DatasetManager:
    ...
    def calculate_statistics(self, split: str = None) -> Dict:
        ...
        for split_name in splits:
            ...
            stats['annotations_per_image'] = {
                ...
                'mean': sum(anns_per_image.values()) / len(anns_per_image) if anns_per_image else 0,
    ...
    def verify_dataset(self, split: str = None) -> Dict:
        ...
        for split_name in splits:
            ...
            if images_without_anns:
                warnings.append(f"Found {len(images_without_anns)} images without annotations")
            ...
            duplicate_ids = [img_id for img_id, count in image_id_counts.items() if count > 1]
            if duplicate_ids:
    ...
    def split_dataset(self, input_split: str = 'train',
    ...
    def _create_split_dataset(self, source_dataset: Dict, images: List[Dict]) -> Dict:
        ...
        # Filter annotations
        annotations = [ann for ann in source_dataset['annotations']
...
def main():
    ...
    parser.add_argument('--split', type=str, default=None,
                       help='Specific split to process')
    ...
    if args.action == 'stats':
        ...
        for split_name, split_stats in stats.items():
            ...
            if 'annotations_per_image' in split_stats:
                ...
                print(f"  Annotations per image: {apm['mean']:.2f} (min: {apm['min']}, max: {apm['max']})")

            if 'bbox_width' in split_stats:
                print(f"  Bbox width: {split_stats['bbox_width']['mean']:.1f} "
                      f"(min: {split_stats['bbox_width']['min']:.1f}, "
                      f"max: {split_stats['bbox_width']['max']:.1f})")

            if 'bbox_height' in split_stats:
                print(f"  Bbox height: {split_stats['bbox_height']['mean']:.1f} "
        ...
        for split_name, split_stats in stats.items():
            print(f"{split_name.upper()}: {split_stats['images']} images, "
