...
def upgrade() -> None:
    ...
    if "recordings" not in existing_tables:
        op.create_table(
            ...
            sa.Column('total_context_events', sa.Integer(), nullable=False, server_default='0'),
            ...
            # Processing status
            sa.Column('status', sa.String(), nullable=False, server_default='uploaded', index=True),
            ...
            sa.Column('discovered_states_count', sa.Integer(), nullable=False, server_default='0'),
            sa.Column('discovered_transitions_count', sa.Integer(), nullable=False, server_default='0'),
            sa.Column('discovered_workflows_count', sa.Integer(), nullable=False, server_default='0'),
        ...
        op.create_index('ix_recordings_project_status', 'recordings', ['project_id', 'status'])
        op.create_index('ix_recordings_created_at', 'recordings', ['created_at'])

    # Create recording_frames table
    if "recording_frames" not in existing_tables:
        op.create_table(
            'recording_frames',
            sa.Column('id', UUID(as_uuid=True), primary_key=True),
            ...
            sa.Column('phash_computed', sa.Boolean(), nullable=False, server_default='false'),
        ...
        op.create_index('ix_recording_frames_recording_frame', 'recording_frames', ['recording_id', 'frame_number'])
        op.create_index('ix_recording_frames_recording_time', 'recording_frames', ['recording_id', 'relative_time_ms'])
        op.create_index('ix_recording_frames_cluster', 'recording_frames', ['recording_id', 'cluster_id'])

    # Create recording_interactions table
    if "recording_interactions" not in existing_tables:
        op.create_table(
        ...
        op.create_index('ix_recording_interactions_recording_time', 'recording_interactions', ['recording_id', 'relative_time_ms'])
        op.create_index('ix_recording_interactions_type', 'recording_interactions', ['recording_id', 'interaction_type'])

    # Create recording_contexts table
    if "recording_contexts" not in existing_tables:
        op.create_table(
            'recording_contexts',
            sa.Column('id', UUID(as_uuid=True), primary_key=True),
            sa.Column('recording_id', UUID(as_uuid=True), nullable=False, index=True),
        ...
        op.create_index('ix_recording_contexts_recording_time', 'recording_contexts', ['recording_id', 'relative_time_ms'])
        op.create_index('ix_recording_contexts_type', 'recording_contexts', ['recording_id', 'event_type'])

    # Create discovered_states table (basic version for recording support)
    # The full version with dual-source support is in migration 20251125_112445
    if "discovered_states" not in existing_tables:
        op.create_table(
            ...
            sa.Column('is_error_state', sa.Boolean(), nullable=False, server_default='false'),
        ...
        op.create_index('ix_discovered_states_recording', 'discovered_states', ['recording_id'])
        op.create_index('ix_discovered_states_confidence', 'discovered_states', ['confidence'])

    # Create discovered_transitions table
    if "discovered_transitions" not in existing_tables:
        op.create_table(
            ...
            sa.Column('stays_visible', sa.Boolean(), nullable=False, server_default='false'),
            ...
            sa.Column('recommended_retry_count', sa.Integer(), nullable=False, server_default='3'),
            ...
            sa.ForeignKeyConstraint(['trigger_interaction_id'], ['recording_interactions.id']),
        )

        # Create indexes for discovered_transitions
        op.create_index('ix_discovered_transitions_recording', 'discovered_transitions', ['recording_id'])
        op.create_index('ix_discovered_transitions_from_state', 'discovered_transitions', ['from_state_id'])
        op.create_index('ix_discovered_transitions_to_state', 'discovered_transitions', ['to_state_id'])

    # Add foreign key constraints (only if tables exist and constraints don't)
    # These are created conditionally to handle idempotency
    try:
        if "recording_frames" in existing_tables and "discovered_states" in existing_tables:
            op.create_foreign_key(
                'fk_recording_frames_state_id',
                'recording_frames', 'discovered_states',
                ['state_id'], ['id'],
                ondelete='SET NULL'
            )
    except Exception:
    ...
    try:
        if "recording_interactions" in existing_tables and "discovered_states" in existing_tables:
            op.create_foreign_key(
                'fk_recording_interactions_target_state_id',
                'recording_interactions', 'discovered_states',
                ['target_state_id'], ['id'],
                ondelete='SET NULL'
            )
    except Exception:
    ...
    try:
        if "recording_interactions" in existing_tables and "discovered_transitions" in existing_tables:
            op.create_foreign_key(
                'fk_recording_interactions_transition_id',
                'recording_interactions', 'discovered_transitions',
                ['transition_id'], ['id'],
                ondelete='SET NULL'
            )
    except Exception:
    ...
    if "processing_logs" not in existing_tables:
        ...
        op.create_index('ix_processing_logs_recording_time', 'processing_logs', ['recording_id', 'timestamp'])
        op.create_index('ix_processing_logs_phase', 'processing_logs', ['recording_id', 'phase'])
...
def downgrade() -> None:
    ...
    op.drop_constraint('fk_recording_interactions_transition_id', 'recording_interactions', type_='foreignkey')
    op.drop_index('ix_discovered_transitions_to_state', table_name='discovered_transitions')
    op.drop_index('ix_discovered_transitions_from_state', table_name='discovered_transitions')
    op.drop_index('ix_discovered_transitions_recording', table_name='discovered_transitions')
    op.drop_table('discovered_transitions')

    # Drop discovered_states constraints (table dropped by other migration)
    op.drop_constraint('fk_recording_interactions_target_state_id', 'recording_interactions', type_='foreignkey')
    op.drop_constraint('fk_recording_frames_state_id', 'recording_frames', type_='foreignkey')

    # Drop recording_contexts
    op.drop_index('ix_recording_contexts_type', table_name='recording_contexts')
    op.drop_index('ix_recording_contexts_recording_time', table_name='recording_contexts')
    ...
    op.drop_index('ix_recording_interactions_recording_time', table_name='recording_interactions')
