"""Add recording tables.

Revision ID: e34e1d177ed8
Revises: 20251124_projects_uuid
Create Date: 2025-11-13 20:00:00.000000

Note: Changed dependency from 63e5da6dd826 to 20251124_projects_uuid
      because this migration uses UUID for project_id foreign key.
"""
...
def upgrade() -> None:  # noqa: C901
    """Create recording-related database tables."""
    ...
    if "recordings" not in existing_tables:
        op.create_table(
            ...
            sa.Column(
                "validation_warnings", JSONB, nullable=False, server_default="[]"
            ),
            # Discovered state structure
            sa.Column(
                "discovered_states_count",
                sa.Integer(),
                nullable=False,
                server_default="0",
            ),
    ...
    if "recording_interactions" not in existing_tables:
        op.create_table(
            ...
            sa.Column(
                "is_sensitive", sa.Boolean(), nullable=False, server_default="false"
            ),
            # Modifiers
    ...
    if "discovered_states" not in existing_tables:
        op.create_table(
            ...
            # State properties
            sa.Column(
                "is_initial", sa.Boolean(), nullable=False, server_default="false"
            ),
            sa.Column(
                "is_error_state", sa.Boolean(), nullable=False, server_default="false"
            ),
            sa.Column(
                "is_transient", sa.Boolean(), nullable=False, server_default="false"
            ),
            # Confidence scores
            ...
            # User review
            sa.Column(
                "user_edited", sa.Boolean(), nullable=False, server_default="false"
            ),
            sa.Column(
    ...
    if "discovered_transitions" not in existing_tables:
        op.create_table(
            ...
            sa.Column(
                "deactivate_state_ids", JSONB, nullable=False, server_default="[]"
            ),
            sa.Column(
    ...
    try:
        if (
            "recording_frames" in existing_tables
            and "discovered_states" in existing_tables
        ):
    ...
    try:
        if (
            "recording_interactions" in existing_tables
            and "discovered_states" in existing_tables
        ):
            op.create_foreign_key(
    ...
    try:
        if (
            "recording_interactions" in existing_tables
            and "discovered_transitions" in existing_tables
        ):
            op.create_foreign_key(
...
def downgrade() -> None:
    """Drop recording-related database tables."""
