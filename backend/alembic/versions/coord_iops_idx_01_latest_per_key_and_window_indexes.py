...
Phase 2/3 verification):
    ...
    SELECT count(*) FILTER (WHERE i.indisvalid) AS valid, count(*) AS present
      FROM pg_index i
      JOIN pg_class c ON c.oid = i.indexrelid
      JOIN pg_namespace n ON n.oid = c.relnamespace
     WHERE n.nspname = 'coord'
       AND c.relname IN ('idx_client_telemetry_obs_best_evidence',
                         'idx_alerts_kind_first_seen_at',
                         'idx_policy_rule_resolutions_repo_pull_repo');

The pass condition is ``valid = 3 AND present = 3``: all three exist AND are
valid, so a missing index cannot read as a pass. A killed build leaves an
INVALID index, and ``IF NOT EXISTS`` would skip it on any later run, so it would
never serve a query. The recovery is a follow-up revision that runs
...
table. Never a plain ``DROP INDEX``. That follow-up has a ``DROP`` on its
upgrade path, so the same classifier will hold it for the ordinary review path;
expect that hold.

Timeouts: the RDS parameter group ``qontinui-staging-pg16`` leaves
``statement_timeout`` at the engine default (0, none; read 2026-09-26), so
nothing at the instance level cuts index 3's build short. A role-level setting
for the migrator's user is not visible from AWS; if the build is cancelled by a
statement timeout, that is where to look.
