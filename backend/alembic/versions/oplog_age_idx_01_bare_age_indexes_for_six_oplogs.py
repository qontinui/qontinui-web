...
# `presetprov01_allow_preset_provisional_profile_source` is `main`'s single
# head at the 2026-09-13 re-point (origin/main `c3566d94a`).
#
# History: this revision was first written against
# `coord_agent_questions_audience_backfill` (2026-09-05). It was then chained
# behind four other open PRs' revisions -- `ptbe_01` (#1218), `devenv_09` /
# `devenv_10` (#989) and `policy_rules_tombstone_01` (#1269) -- to serialise a
# five-way fork. Every revision in that chain is on `main` now, so the chain has
# no remaining purpose and this revision re-points at the live head. `pdtier_03`
# (#1180) is still chained behind this one. It is the only revision in that set
# that drops a column or can `RAISE EXCEPTION` mid-upgrade, which is why it goes
# after this purely additive, fully reversible one.
#
# A parent that has landed but is no longer the head forks exactly as a
# never-landed one does. If another revision lands before this one, re-point
...
down_revision: str | None = "presetprov01_allow_preset_provisional_profile_source"
