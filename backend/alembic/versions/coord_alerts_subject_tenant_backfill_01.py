...
Revises: coord_pg_overload_idx_03, tagpurge_01_phantom_tag_branch_rows

Also merges two pre-existing heads
----------------------------------
``origin/main`` carried TWO alembic heads with no merge between them —
``coord_pg_overload_idx_03`` and ``tagpurge_01_phantom_tag_branch_rows`` — which
``alembic upgrade head`` (singular, used by ``backend/start-backend.sh``) rejects
with "Multiple head revisions are present". This migration takes BOTH as its
down_revision, so it both merges the branches back to a single head AND applies
the backfill below. (Reducing 2 heads to 1 cannot regress the migration state.)
...
# Tuple → also merges the two pre-existing heads back to one (see docstring).
down_revision = ("coord_pg_overload_idx_03", "tagpurge_01_phantom_tag_branch_rows")
branch_labels = None
depends_on = None
