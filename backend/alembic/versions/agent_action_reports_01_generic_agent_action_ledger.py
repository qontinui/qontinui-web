...
Revises: coord_prompt_docs_02_drop
...
Chains off ``coord_prompt_docs_02_drop``, the single live head of the alembic
chain on ``main`` (computed from the repo, not from the plan). If a concurrent
head-race moves ``main``'s head before this lands, re-point ``down_revision``
onto the new head.

Re-pointed 2026-07-31 from the original ``operators_disabled_01``: while this PR
sat open, ``main`` advanced past that revision and the chain forked in two. The
``alembic-heads-pr`` gate that went red on 2026-07-30 named the fork as
``agent_action_reports_01`` vs ``coord_sesscompl_03``; by the time of this
re-point ``main`` had advanced further still, through ``memhold_backfill_01``
and ``devenv_07_org_sharing`` to ``coord_prompt_docs_02_drop`` — so the head
named here is the current one, not the one in that 2026-07-30 log.

The fix is this re-point plus a rebase onto ``main`` — NOT an ``alembic merge``
revision, since nothing had landed that this migration must join; a merge
revision would have preserved the fork in history for no benefit.
"""
...
down_revision: str | Sequence[str] | None = "coord_prompt_docs_02_drop"
