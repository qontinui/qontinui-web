...
``down_revision`` is ``drr_01``, the repo's **single** head. It was
originally authored onto ``policy_rules_tombstone_01``, which was the single
head that day; a sibling landed underneath while this revision was open and
CI's ``alembic-graph`` notifier flagged the resulting 2-head fork, so it was
re-pointed onto the landed head. That is the prescribed fix, and NOT
``alembic merge`` — a merge revision is correct only when both heads have
already landed, whereas re-pointing an unlanded revision leaves nothing behind.
This repo keeps strict single-head discipline — ``scripts/ci/count_alembic_heads.py`` gates it, and the
...
down_revision: str | Sequence[str] | None = "drr_01"
