...
* ``coord.pr_events``, ``coord.pr_labels``, ``coord.plan_pr_citations`` —
  derived from the row's ``repo`` via ``coord.tenant_repos`` (the
  ``owner/name``-keyed repo→tenant ownership map). For a repo owned by
  multiple tenants the LOWEST tenant_id is picked deterministically — the
  same "first owner" convention the Rust write paths use
  (``repo_branches.rs``: ``SELECT tenant_id FROM coord.tenant_repos WHERE
  repo = $1 LIMIT 1``).
...
# Deterministic first-owner pick per repo (lowest tenant_id), mirroring the
# Rust write paths' LIMIT-1 convention but stable across runs.
_OWNER_CTE = (
...
def upgrade() -> None:
    for table in ("pr_events", "pr_labels", "plan_pr_citations"):
        op.execute(
            f"""
