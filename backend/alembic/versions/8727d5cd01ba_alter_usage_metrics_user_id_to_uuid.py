...
def upgrade() -> None:
    ...
    op.execute("ALTER TABLE usage_metrics ALTER COLUMN user_id TYPE UUID USING user_id::text::uuid")


def downgrade() -> None:
    # Downgrade would convert back to INTEGER (not recommended, but for completeness)
    op.execute("ALTER TABLE usage_metrics ALTER COLUMN user_id TYPE INTEGER USING EXTRACT(EPOCH FROM user_id::text::timestamp)::integer")
