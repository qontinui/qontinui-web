...
def upgrade() -> None:
    ...
    op.add_column('audit_logs', sa.Column('event_category', sa.String(), nullable=True, comment='Category: permission_change, membership_change, pii_access, account_modification, etc.'))
    op.add_column('audit_logs', sa.Column('correlation_id', sa.String(), nullable=True, comment='Request correlation ID for tracing related events'))
    op.add_column('audit_logs', sa.Column('target_user_id', postgresql.UUID(as_uuid=True), nullable=True, comment='User being affected by the action (for permission/membership changes)'))
    op.add_column('audit_logs', sa.Column('changes', postgresql.JSON(astext_type=sa.Text()), nullable=True, comment='Before/after state for modifications as {before: {...}, after: {...}}'))

    # Add foreign key constraint for target_user_id
    op.create_foreign_key('fk_audit_logs_target_user', 'audit_logs', 'users', ['target_user_id'], ['id'], ondelete='SET NULL')
    ...
    op.create_index('ix_audit_logs_target_user', 'audit_logs', ['target_user_id', 'created_at'])
