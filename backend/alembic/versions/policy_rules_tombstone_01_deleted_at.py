...
# `coord_test_results_idx_01` is `main`'s CURRENT head (2026-09-12).
#
# This line previously read `devenv_10_unique_active_coord_device`, with a long
# comment explaining that the parent was an UNLANDED sibling (qontinui-web
# **#989**) and that `alembic-heads-pr` was therefore RED BY CONSTRUCTION,
# going green on its own once three PRs ahead of it landed, in this order:
#
#   #1210 -> #1218 -> #989 -> #1269 (this PR) -> #1216 -> #1180
#
# That premise expired. **#1210, #1218, #989 and #1216 all closed UNMERGED**
# (`state=closed, merged=false`, checked 2026-09-12), so nothing in that order
# ever landed as written. Their revisions did reach `main` by other routes, so
# `devenv_10_unique_active_coord_device` IS on `main` today -- but it is no
# longer the head: `main` has since moved three revisions past it
# (`devenv_10` -> `claude_acct_01` -> `plan_library_05_scan_root_observations`
# -> `coord_test_results_idx_01`). A parent that is present but is not the head
# forks the graph permanently, so the red stopped being self-clearing and
# waiting became a no-op. Re-pointed by hand onto the head instead.
#
# The original comment's warning still stands and is why ONLY this revision --
# the chain HEAD -- moved: re-pointing every forked sibling at `main`'s head
# would re-fork them the moment the first one lands. The rest of this chain is
# untouched and its total order survives, because both links name revision ids
# rather than `main`:
#
#   coord_test_results_idx_01 (main) -> policy_rules_tombstone_01 (#1269, here)
#     -> oplog_age_idx_01 (#1272) -> pdtier_03 (#1180)
down_revision: str = "coord_test_results_idx_01"
