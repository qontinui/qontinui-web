...
def upgrade() -> None:
    ...
    if "project_locks" not in existing_tables:
        op.create_table(
            "project_locks",
            sa.Column(
                "id",
        ...
        op.create_index(
            "ix_project_locks_resource_id", "project_locks", ["resource_id"]
        )
    ...
    if "project_comments" not in existing_tables:
        op.create_table(
            ...
            sa.Column(
                "position", postgresql.JSON(astext_type=sa.Text()), nullable=True
            ),
            sa.Column(
                "mentions", postgresql.JSON(astext_type=sa.Text()), nullable=True
            ),
            sa.Column(
                "resolved",
                sa.Boolean(),
        ...
        op.create_index(
            "ix_project_comments_action_id", "project_comments", ["action_id"]
        )
        op.create_index(
        ...
        op.create_index(
            "ix_project_comments_resolved", "project_comments", ["resolved"]
        )
        op.create_index(
    ...
    if "activity_logs" not in existing_tables:
        ...
        op.create_index(
            "ix_activity_logs_resource_id", "activity_logs", ["resource_id"]
        )
        op.create_index("ix_activity_logs_created_at", "activity_logs", ["created_at"])

    # Create composite indexes for common queries (only if tables were just created)
    if "project_locks" not in existing_tables:
        op.create_index(
    ...
    if "conflict_logs" not in existing_tables:
        op.create_table(
            "conflict_logs",
            sa.Column(
                "id",
            ...
            sa.Column(
                "base_data", postgresql.JSON(astext_type=sa.Text()), nullable=True
            ),
            sa.Column(
                "local_data", postgresql.JSON(astext_type=sa.Text()), nullable=True
            ),
            sa.Column(
                "remote_data", postgresql.JSON(astext_type=sa.Text()), nullable=True
            ),
            sa.Column("changes", postgresql.JSON(astext_type=sa.Text()), nullable=True),
            sa.Column(
                "detected_at",
            ...
            sa.ForeignKeyConstraint(
                ["local_user_id"], ["users.id"], ondelete="CASCADE"
            ),
            sa.ForeignKeyConstraint(
                ["remote_user_id"], ["users.id"], ondelete="CASCADE"
            ),
            sa.PrimaryKeyConstraint("id"),
        )
        # Create indexes for conflict_logs
        op.create_index(
            "ix_conflict_logs_resource_id", "conflict_logs", ["resource_id"]
        )
        op.create_index(
            "ix_conflict_logs_local_user_id", "conflict_logs", ["local_user_id"]
        )
        op.create_index(
            "ix_conflict_logs_remote_user_id", "conflict_logs", ["remote_user_id"]
        )
