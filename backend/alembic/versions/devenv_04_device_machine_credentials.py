...
(``coord_test_targets`` after rebasing onto origin/main, which brought in
the fleet-fresh migration chain). Re-pointed from the authoring-time parent
``devenv_03_coord_device_bridge`` to resolve the migration fork; coord also
re-points at land time and ``alembic-graph-pr`` CI guards forks.
"""
...
down_revision = "coord_test_targets"
branch_labels = None
depends_on = None
