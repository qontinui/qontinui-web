...
from models.button_cnn import create_button_cnn, ButtonCNN
from models.button_yolo import create_button_yolo, ButtonYOLO
...
class ButtonDetectorTrainer:
    ...
    def train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        for batch_idx, (images, targets) in enumerate(self.train_loader):
            ...
            is_button_loss = self.criterion_classification(
                outputs['is_button'],
                targets['is_button'].to(self.device)
            )

            button_type_loss = self.criterion_classification(
                outputs['button_type'],
                targets['button_type'].to(self.device)
            )
            ...
            confidence_loss = self.criterion_confidence(
                outputs['confidence'],
                confidence_target.to(self.device)
            )
    ...
    def validate(self, epoch: int) -> Dict[str, float]:
        ...
        for images, targets in self.val_loader:
            ...
            correct_is_button += is_button_pred.eq(targets['is_button'].to(self.device)).sum().item()
            correct_button_type += button_type_pred.eq(targets['button_type'].to(self.device)).sum().item()
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.config.get('epochs', 50)):
            ...
            self.writer.add_scalar('val/is_button_accuracy', val_metrics['is_button_accuracy'], epoch)
            self.writer.add_scalar('val/button_type_accuracy', val_metrics['button_type_accuracy'], epoch)
...
def main():
    ...
    parser.add_argument('--train-annotations', type=str, help='Path to training annotations')
    parser.add_argument('--val-annotations', type=str, help='Path to validation annotations')
    parser.add_argument('--checkpoint-dir', type=str, default='checkpoints', help='Checkpoint directory')
    parser.add_argument('--pretrained', action='store_true', help='Use pretrained weights')
    parser.add_argument('--num-classes', type=int, default=4, help='Number of button classes')
