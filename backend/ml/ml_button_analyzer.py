...
class MLButtonAnalyzer(BaseAnalyzer):
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        print(
            f"MLButtonAnalyzer initialized: {self.inference.model_type} on {self.inference.device}"
        )
    ...
    async def analyze(self, input_data: AnalysisInput) -> AnalysisResult:
        ...
        for idx, (screenshot_meta, screenshot_bytes) in enumerate(
            ...
            if self.mode == "detection" and isinstance(
                self.inference.model, type(self.inference.model)
            ):
    ...
    def analyze_regions(
        ...
        for pred in predictions:
            if (
                pred["is_button"]
                and pred["confidence"] >= self.inference.confidence_threshold
            ):
                bbox = BoundingBox(
                    x=pred["bbox"][0],
                    y=pred["bbox"][1],
