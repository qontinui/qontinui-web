...
class ButtonDatasetGenerator:
    ...
    LABELS = [
        'Submit', 'Cancel', 'OK', 'Apply', 'Close', 'Save', 'Delete',
        'Edit', 'Add', 'Remove', 'Search', 'Filter', 'Export', 'Import',
        'Upload', 'Download', 'Share', 'Print', 'Copy', 'Paste', 'Cut',
        'Undo', 'Redo', 'Refresh', 'Settings', 'Help', 'Next', 'Previous',
        'Start', 'Stop', 'Play', 'Pause', 'Login', 'Logout', 'Register',
        'Confirm', 'Retry', 'Skip', 'Continue', 'Finish', 'Back', 'Home'
    ]
    ...
    def _adjust_color_for_state(self, color: Tuple[int, int, int], state: str) -> Tuple[int, int, int]:
    ...
    def _draw_button(self, draw: ImageDraw.Draw, x: int, y: int, width: int, height: int,
                     style: str, color: Tuple[int, int, int], shape: str, state: str,
                     label: str) -> None:
        ...
        if style == 'flat':
            self._draw_rounded_rectangle(draw, x, y, x + width, y + height, radius,
                                         fill=button_color)

        elif style == '3d':
            # Draw shadow
            shadow_color = tuple(int(c * 0.6) for c in button_color)
            self._draw_rounded_rectangle(draw, x + 2, y + 3, x + width + 2, y + height + 3,
            ...
            if state != 'pressed':
                ...
                draw.line([(x + radius, y + 1), (x + width - radius, y + 1)],
            ...
            draw.line([(x, y + height), (x, y), (x + width, y)], fill=highlight, width=2)
            draw.line([(x + width, y), (x + width, y + height), (x, y + height)],
                     fill=shadow, width=2)
    ...
    def _draw_rounded_rectangle(self, draw: ImageDraw.Draw, x1: int, y1: int,
                                x2: int, y2: int, radius: int, **kwargs) -> None:
    ...
    def _generate_background(self, width: int, height: int, context: str) -> Image.Image:
        ...
        if context == 'toolbar':
            # Add toolbar border
            draw.line([(0, height - 1), (width, height - 1)], fill=(209, 213, 219), width=1)

        elif context == 'dialog':
            # Add dialog border
            draw.rectangle([0, 0, width - 1, height - 1], outline=(209, 213, 219), width=1)
            # Title bar
            draw.rectangle([0, 0, width, 40], fill=(243, 244, 246))

        elif context == 'card':
            # Card with shadow
            draw.rectangle([0, 0, width - 1, height - 1], outline=(229, 231, 235), width=1)
        ...
        for i in range(num_buttons):
            ...
            for attempt in range(max_attempts):
                ...
                for bx, by, bw, bh in placed_buttons:
                    if not (x + width < bx or x > bx + bw or
                           y + height < by or y > by + bh):
                        overlap = True
                        break
            ...
            # Draw button
            self._draw_button(draw, x, y, width, height, style, color, shape, state, label)
    ...
    def generate_dataset(self, num_samples: int, train_ratio: float = 0.7,
                        val_ratio: float = 0.15, test_ratio: float = 0.15) -> Dict[str, int]:
        """
...
def main():
    ...
    parser.add_argument('--output-dir', type=str, default='datasets/buttons',
    ...
    for split, data in stats.items():
        print(f"  {split.upper():5s}: {data['samples']:4d} samples, "
