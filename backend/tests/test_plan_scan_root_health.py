...
class TestComparisonSet:
    ...
    def test_two_floors_on_one_ref_with_nothing_ahead_are_ordered(self) -> None:
        ...
        # One shared ref nobody fetched fresh: staleness IS the cause here.
    ...
    def test_a_zero_floor_beside_an_exact_count_on_another_ref_is_unknown(
        ...
        # Two refs: the floor is incomparability, whatever either ref's age.
        assert rollup.detail is not None
        assert rollup.detail.startswith("refs_not_shared:")

    def test_an_exact_zero_on_its_own_fresh_ref_beside_another_ref_is_not_ref_stale(
        self,
    ) -> None:
        ...
        rollup = _only_rollup(in_step, behind)
...
class TestOnlyOrderedStandingIsNamed:
    ...
    def test_a_stale_shared_ref_with_an_unknown_ahead_places_nobody(self) -> None:
        ...
        its own, which is exactly what makes a stale ref unorderable. (The
        write door refuses such a report, so a stored one is corrupt.)"""
