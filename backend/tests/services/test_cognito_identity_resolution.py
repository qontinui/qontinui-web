...
class TestMalformedPoolRows:
    def test_a_user_with_no_sub_raises_rather_than_reading_as_absent(self, paging):
        ...
        one answer that must NOT come back is ``None``: for a superuser the
        route turns that into an invitation, creating a second account for a
        colleague the pool just returned."""
