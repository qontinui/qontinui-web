...
class TestDeviceMachineCredentialCrud:
    ...
    async def test_mint_then_get_by_hash(
    ...
    async def test_revoke_invalidates(
        self, async_db_session: AsyncSession
    ) -> None:
        ...
        assert (
            await dmk_crud.get_by_hash(async_db_session, original_hash)
        ) is None
    ...
    async def test_bump_last_used_slides_expiry(
        ...
        await dmk_crud.bump_last_used(
            async_db_session, cred, slide_ttl_days=60
        )
