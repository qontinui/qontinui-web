...
class GUIScreenshotGenerator:
    ...
    def generate_button(
        ...
        # Button background
        draw.rectangle(
            [x, y, x + width, y + height], fill=color, outline="#2E7D32", width=2
        )
    ...
    def generate_text_input(
        ...
        # Input background
        draw.rectangle(
            [x, y, x + width, y + height], fill="white", outline="#CCCCCC", width=2
        )
    ...
    def generate_dialog_box(
        ...
        # Dialog background
        draw.rectangle(
            [x, y, x + width, y + height], fill="#F5F5F5", outline="#999999", width=3
        )
    ...
    def generate_sidebar(self, draw: ImageDraw.ImageDraw, items: list[str]) -> None:
        ...
        for i, item in enumerate(items):
            y = 50 + i * item_height
            draw.rectangle(
                [5, y, sidebar_width - 5, y + item_height - 5], fill="#616161"
            )
    ...
    for set_idx in range(count):
        ...
        for screenshot_idx in range(screenshots_per_set):
            ...
            url = await upload_file(
                filename, img_bytes.getvalue(), content_type="image/png"
            )
...
async def main():
    ...
    parser.add_argument(
        "--count", type=int, default=3, help="Number of annotation sets to generate"
    )
    parser.add_argument(
        "--screenshots",
        type=int,
    ...
    async with AsyncSessionLocal() as db:
        ...
        annotation_sets = await generate_test_annotation_sets(
            db, user, count=args.count, screenshots_per_set=args.screenshots
        )
